...
export const fileTraceProgress = {
  ...
  "src/pages/api/news.ts": {
    ...
    nextPendingSourceFile: "src/pages/api/users/me/notifications.ts",
    nextPendingMethodId: "src/pages/api/users/me/notifications.ts#GET",
  },
  "src/pages/api/news/[slug].ts": {
    batchId: "phase4a1-trace-batch-6",
    methodIds: ["src/pages/api/news/[slug].ts#GET"],
    status: "complete",
    completedMethodIds: ["src/pages/api/news/[slug].ts#GET"],
    pendingMethodIds: [],
    historicalBlockerId: "PUBLIC_NEWS_DETAIL_SLUG_AND_OUTPUT_SAFETY",
    runtimeRemediationStatus: "implemented-source-reaudited",
    routeClient: "anon-key-only createSSRClient without bearer propagation",
    firstDatabaseOperation: "getPublicNewsArticleBySlug#exact news_articles slug/status published maybeSingle select",
    firstExternalEffect: "none",
    firstPersistentEffect: "none",
    providerArchitecture: "none; this route has no RSS, provider API, fetch wrapper, image fetch, or outbound request",
    cacheArchitecture: "none; no memory, KV, database, filesystem, or external cache lookup/write",
    publicOutputContract: "published-only normalized article and bounded related rows with safe URLs, bounded text, and author_id removed",
    nextPendingSourceFile: "src/pages/api/users/me/notifications.ts",
    nextPendingMethodId: "src/pages/api/users/me/notifications.ts#GET",
  },
  "src/pages/api/users/me/notifications.ts": {
    batchId: "phase4a1-trace-batch-6",
    methodIds: ["src/pages/api/users/me/notifications.ts#GET", "src/pages/api/users/me/notifications.ts#PATCH"],
    status: "complete",
    completedMethodIds: ["src/pages/api/users/me/notifications.ts#GET", "src/pages/api/users/me/notifications.ts#PATCH"],
    pendingMethodIds: [],
    authentication: "single strict Bearer token, bearer-bound anon-key RLS client, auth.getUser verified recipient",
    pagination: "limit defaults to 20 and is bounded to canonical decimal 1-50; only unread_only true/false/1/0 is accepted; no offset/cursor/type/date/sort grammar exists",
    readIsolation: "every notification count, list, and update repeats recipient_id = verified auth.getUser id; active RLS independently requires recipient_id = auth.uid",
    payloadContract: "minimal notification fields only; recipient, actor, post, comment, raw avatar, metadata, profile role, safety, moderation, report, and storage fields are omitted",
    lifecycleRule: "hidden, deleted, missing, inactive, canonical-hidden, or mismatched target ancestry keeps the notification shell but redacts preview and falls back to /notifications/",
    getFirstDatabaseOperation: "getUnreadCount and loadNotificationRows parallel recipient-bound forum_notifications reads after strict authentication and query validation",
    getFirstIrreversibleEffect: "none",
    patchFirstPersistentEffect: "forum_notifications.update(read_at) after strict authenticated action validation",
    patchIdempotency: "recipient_id plus read_at IS NULL makes repeat mark_read/mark_all_read no-op for already-read rows",
    rlsMigrationRequired: false,
    nextPendingSourceFile: "src/pages/api/users/me/profile.ts",
    nextPendingMethodId: "src/pages/api/users/me/profile.ts#POST",
  },
  "src/pages/api/users/me/profile.ts": {
    batchId: "phase4a1-trace-batch-6",
    methodIds: ["src/pages/api/users/me/profile.ts#POST"],
    status: "complete",
    completedMethodIds: ["src/pages/api/users/me/profile.ts#POST"],
    pendingMethodIds: [],
    authentication: "strict Bearer token, anon-key bearer-bound RLS client, and auth.getUser verified actor only",
    acceptedPayloadFields: ["display_name", "username", "bio", "avatar_url", "banner_url"],
    firstDatabaseOperation: "assertUserCanWrite#getUserSafetyState user_safety_states read scoped to verified auth.userId",
    firstExternalEffect: "moderateContent after verified actor, safety, strict payload/media validation, and current-profile read",
    firstPersistentEffect: "profiles.update scoped to id = auth.userId",
    usernameIntegrity: "NFC trim and lower-case route normalization plus profiles_username_unique_ci lower(username) database uniqueness; unique violations return PROFILE_USERNAME_UNAVAILABLE",
    mediaIntegrity: "submitted profile media must be an exact verified-user and matching-kind canonical key before signing or moderation",
    serviceRoleAudit: "test:profile-audit remains 48/1 because legal-consent-repository.server.ts is unrelated and not imported by this route",
    nextPendingSourceFile: "src/pages/api/users/me/summary.ts",
    nextPendingMethodId: "src/pages/api/users/me/summary.ts#GET",
  },
  "src/pages/api/users/me/summary.ts": {
    batchId: "phase4a1-trace-batch-6",
    methodIds: ["src/pages/api/users/me/summary.ts#GET"],
    status: "complete",
    completedMethodIds: ["src/pages/api/users/me/summary.ts#GET"],
    pendingMethodIds: [],
    authentication: "strict Bearer token, anon-key bearer-bound RLS client, and auth.getUser verified actor only",
    returnedFields: ["profile.id", "profile.username", "profile.display_name", "profile.profile_href", "profile.avatar_resolved_url", "stats.post_count", "stats.received_like_count"],
    firstDatabaseOperation: "loadProfile#profiles.select(id,username,display_name,avatar_url).eq(id,auth.userId)",
