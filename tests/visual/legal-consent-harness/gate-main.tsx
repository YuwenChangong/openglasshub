...
function App() {
 ...
 const auth=useMemo(()=>({getSession:async()=>{add("session"); if(id==="gate-session-loading") return await new Promise<null>(()=>{}); return signedOut?null:{accessToken:"memory-only"};},subscribe:()=>()=>{}}),[id,signedOut]);
 const consent=useMemo(()=>({getCurrentConsent:async()=>{add("consent"); if(id==="gate-consent-loading") return await new Promise<LegalConsentStatus>(()=>{}); if(id.includes("failure")) throw new LegalConsentClientError("UNAVAILABLE"); if(id==="gate-rate-limited") throw new LegalConsentClientError("RATE_LIMITED"); if(id.includes("missing")||id.includes("outdated")) return {current:false,bundleVersion:"test",consentUrl:"/legal-consent/"}; return {current:true,bundleVersion:"test",consentUrl:"/legal-consent/"};},recordCurrentConsent:async()=>({current:true,bundleVersion:"test",consentUrl:"/legal-consent/"})}),[id]);
