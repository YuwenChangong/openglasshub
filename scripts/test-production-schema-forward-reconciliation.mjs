...
assert.equal(manifest.securityFindingCount, 132);
assert.equal(manifest.actionableManifestItemCount, 147);
assert.equal(manifest.uniqueRepairObjectCount, 68);
assert.deepEqual(manifest.productionExecutionCounts, {
  productionAppliedManifestItemCount: 21,
  productionAppliedRepairObjectCount: 7,
  pendingManifestItemCount: 147,
  pendingRepairObjectCount: 68,
  pendingSecurityFindingCount: 132,
});
...
assert.equal(operationalWave?.status, "INDEX_STAGE_A_APPLIED_POSTFLIGHT_VERIFIED_STAGE_B_REVIEW_READY_POLICY_PRIVILEGE_HOLD");
...
assert.deepEqual(Object.fromEntries(["P0_SECURITY_BROADENING", "P1_REQUIRED_SECURITY_OBJECT_MISSING", "P2_SECURITY_AVAILABILITY_DIVERGENCE", "P3_NON_SECURITY_SCHEMA_DRIFT"].map((severity) => [severity, activeItems.filter((item) => item.severity === severity).length])), {
  ...
  P1_REQUIRED_SECURITY_OBJECT_MISSING: 110,
...
assert.match(plan, /68 pending logical repair objects/i);
assert.match(plan, /CIRCLES_VISIBILITY_FOUNDATION/);
assert.match(plan, /W6 operational guardrails[\s\S]*INDEX_STAGE_A_APPLIED_POSTFLIGHT_VERIFIED_STAGE_B_REVIEW_READY_POLICY_PRIVILEGE_HOLD/);
