...
try {
  ...
  const baseline = JSON.parse(await readFile(result.evidence.baselineManifest.path, "utf8"));
  const terminal = JSON.parse(await readFile(result.evidence.baselineTerminal.path, "utf8"));
  assert.equal(baseline.schemaVersion, "legal-local-prelegal-baseline-manifest-v1"); assert.equal(baseline.runtimeClassification, "R6_LOCAL_PRELEGAL_BASELINE_REQUIRES_LOCAL_SUPABASE_RUNTIME");
  assert.equal(terminal.schemaVersion, "legal-local-prelegal-baseline-terminal-v1"); assert.equal(terminal.executed, 0); assert.equal(result.evidence.journal, undefined);
