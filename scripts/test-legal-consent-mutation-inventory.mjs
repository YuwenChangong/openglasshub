...
const commentReactionVisibilityBlocker = releaseBlockingFindings.find((finding) => finding?.id === "COMMENT_REACTION_PARENT_POST_VISIBILITY_AUTHORIZATION");
assert.equal(releaseBlockingFindings.length, 1);
  assert.equal(commentReactionVisibilityBlocker?.status, "active");
  assert.equal(commentReactionVisibilityBlocker?.remediationImplemented, true);
  assert.equal(commentReactionVisibilityBlocker?.reAuditRequired, true);
assert.equal(commentReactionVisibilityBlocker?.sourceFile, "src/pages/api/forum/comments.ts");
assert.equal(commentReactionVisibilityBlocker?.method, "PUT");
assert.equal(commentReactionVisibilityBlocker?.routeEvidence?.symbol, "resolveAccessibleCommentReactionTarget");
assert.equal(commentReactionVisibilityBlocker?.rlsEvidence?.symbol, "can_access_comment_reaction_target");
assert.equal(commentReactionVisibilityBlocker?.mismatchEvidence?.symbol, "GET");
assert.equal(commentReactionVisibilityBlocker?.singleLayerFixInsufficient, true);
assert.equal(commentReactionVisibilityBlocker?.requiresRuntimeRemediation, true);
assert.equal(commentReactionVisibilityBlocker?.requiresForwardMigrationRemediation, true);
assert.equal(commentReactionVisibilityBlocker?.consentDoesNotRemediateAuthorization, true);
