...
const resolvedCommentReactionVisibility = resolvedSecurityFindings.find((finding) => finding?.id === "COMMENT_REACTION_PARENT_POST_VISIBILITY_AUTHORIZATION");
  ...
  const commentReadCircleVisibility = releaseBlockingFindings.find((finding) => finding?.id === "COMMENT_READ_CIRCLE_ANCESTOR_VISIBILITY_AUTHORIZATION");
  assert.equal(releaseBlockingFindings.length, 1);
  assert.equal(commentReadCircleVisibility?.status, "active");
  assert.equal(commentReadCircleVisibility?.sourceFile, "src/pages/api/forum/comments.ts");
  assert.equal(commentReadCircleVisibility?.method, "GET");
  assert.equal(commentReadCircleVisibility?.requiresRuntimeRemediation, true);
  assert.equal(commentReadCircleVisibility?.requiresForwardMigrationRemediation, true);
  ...
  assert.equal(commentReadCircleVisibility?.firstDatabaseOperation?.evidenceType, "first-database-read");
  assert(commentReadCircleVisibility?.evidence?.every((entry) => entry.sourceFile && entry.symbol && entry.evidenceType && entry.conciseFinding));
