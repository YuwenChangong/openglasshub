...
class Adapter {
  constructor({ ambiguousPost = false, commentFailure = false } = {}) { this.calls = []; this.ambiguousPost = ambiguousPost; this.commentFailure = commentFailure; this.post = null; this.comment = null; }
...
{ const value = journal(); const store = new MemoryStore(); await store.write(value); const adapter = new Adapter(); const complete = await executeMinimalCanary({ adapter, store, journal: value }); assert.equal(complete.state, "COMPLETE_TWO_WRITES"); assert.deepEqual(adapter.calls, ["createPost", "createComment"]); assert.equal(store.writes[1].state, "POST_SUBMISSION_PENDING"); }
{ const value = journal(); const store = new MemoryStore(); await store.write(value); const adapter = new Adapter({ commentFailure: true }); await assert.rejects(executeMinimalCanary({ adapter, store, journal: value }), /comment failure/); assert.equal((await store.read()).state, "PARTIAL_POST_CREATED_COMMENT_FAILED"); assert.deepEqual(adapter.calls, ["createPost", "createComment"]); }
...
console.log("PRODUCTION_MINIMAL_CANARY_V3_OK persistent two-write, zero-retry, and partial-completion tests passed with zero network");
