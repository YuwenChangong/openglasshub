...
const container = (id, labelSet = labels(taskId), overrides = {}) => ({ id, labels: labelSet, running: true, imageId: "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", networks: { [`openglass-${taskId}`]: {} }, mounts: [{ Type: "volume", Name: `openglass-${taskId}` }], ...overrides });
...
function fakeDocker(containers, context = "desktop-linux") {
  return (command, args) => {
    ...
    if (args[0] === "inspect") {
      ...
      return `${JSON.stringify(value.labels)}\t${value.running}\t${value.imageId}\t${JSON.stringify(value.networks)}\t${JSON.stringify(value.mounts)}\n`;
    }
...
console.log(JSON.stringify({ classification: "NORMALIZED_REPLAY_TASK_SCOPING_FIXTURES_PASSED", scenarios: 12, realDockerOperations: 0 }));
