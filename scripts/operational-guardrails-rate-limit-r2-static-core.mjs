...
export function assertR2ProposalStaticContract(text) {
  ...
  requirePattern(text, /v_current_bytes numeric;[\s\S]*SELECT pg_catalog\.count\(\*\)[\s\S]*IF v_current_count >= v_max_attempts THEN[\s\S]*IF v_external_video_daily_bytes THEN[\s\S]*SELECT COALESCE\(pg_catalog\.sum\(bytes\), 0::numeric\)[\s\S]*purpose = 'external_video_upload'[\s\S]*created_at >= v_now - INTERVAL '24 hours'[\s\S]*v_current_bytes > 314572800 - p_bytes[\s\S]*INSERT INTO public\.forum_upload_attempts/s, "count, rolling byte quota, and accepted insert are not ordered atomically");
  requirePattern(text, /PERFORM pg_catalog\.pg_advisory_xact_lock[\s\S]*SELECT pg_catalog\.count\(\*[\s\S]*SELECT COALESCE\(pg_catalog\.sum\(bytes\), 0::numeric\)[\s\S]*INSERT INTO public\.forum_upload_attempts/s, "external-video byte sum is not protected by the shared-IP lock");
