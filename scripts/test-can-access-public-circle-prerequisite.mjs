...
assert.match(report, /ADDITIONAL_PREFLIGHT_REQUIRED/);
assert.match(report, /No proposal or postflight has been authored/);
...
assert.doesNotMatch(trackedFiles, /(?:production-schema-fingerprint\.csv|increment-post-view-count-body\.csv)/i, "production exports must remain untracked");

console.log(JSON.stringify({ localDockerOnly: true, expectedFunctionHash: functionDefinition.deterministicSha256, productionDependencyState: "ADDITIONAL_READ_ONLY_PREFLIGHT_REQUIRED", proposalAuthored: false, realProductionOperations: 0 }));
