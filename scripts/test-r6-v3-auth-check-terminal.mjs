...
const failure = base(); failure.success = false; failure.outerClassification = "R6_CURRENT_CANONICAL_V3_AUTH_CHECK_ONLY_FAILED"; failure.innerClassification = "R6_CURRENT_CANONICAL_V3_AUTH_CHECK_CAPTURE_TERMINAL_FRESHNESS_INVALID"; failure.failureStage = "capture_terminal_freshness"; failure.exceptionType = "System.Management.Automation.RuntimeException"; failure.childExitCode = 1; failure.captureTerminalFreshnessAccepted = false; failure.captureProvenancePassed = false; failure.attestationPathMatched = false; failure.attestationShaMatched = false; failure.authenticationAttempted = false; failure.authenticationCompleted = false; failure.sessionCreated = false; failure.sessionValidated = false; failure.authenticatedCheckReached = false; failure.authenticatedCheckCompleted = false; failure.childStarted = false;
assert.equal(validateR6V3AuthCheckTerminal(failure).classification, failure.outerClassification);
for (const [name, mutate] of Object.entries({ missingProvenance: (v) => { delete v.captureProvenancePassed; }, ambiguousFailure: (v) => { v.success = false; v.outerClassification = "R6_CURRENT_CANONICAL_V3_AUTH_CHECK_ONLY_FAILED"; v.innerClassification = "R6_CURRENT_CANONICAL_V3_AUTH_CHECK_UNEXPECTED_FAILURE"; v.failureStage = "capture_terminal_locate"; v.exceptionType = "System.Exception"; v.childExitCode = 1; }, numericString: (v) => { v.remainingValidityMs = "899000"; } })) {
  const value = base(); mutate(value); assert.throws(() => validateR6V3AuthCheckTerminal(value), /^Error: R6_V3_AUTH_CHECK_TERMINAL_/i, name);
}
