...
${users.map((user) => `INSERT INTO auth.identities (id,provider_id,user_id,identity_data,provider,email,last_sign_in_at,created_at,updated_at) VALUES (${sqlLiteral(randomUUID())}::uuid,${sqlLiteral(user.email)},${sqlLiteral(user.id)}::uuid,${sqlLiteral(JSON.stringify({ sub: user.email, email: user.email }))}::jsonb,'email',${sqlLiteral(user.email)},now(),now(),now());`).join("\n")}
