...
async function fixture() {
  ...
  await writeFile(executionPackagePath, JSON.stringify(executionManifest));
  await writeFile(packageManifestPath, JSON.stringify({ schemaVersion: PACKAGE_MANIFEST_VERSION, implementationCommit: commit, transportContractVersion: TRANSPORT_CONTRACT_VERSION, executionPackageArtifact: path.basename(executionPackagePath), executionPackageSha256: hash(await readFile(executionPackagePath)), migration: executionManifest.migration, postflight: executionManifest.postflight, targetProbe: executionManifest.targetProbe, targetIdentitySha256: "5f03b39617d42cf3d1611488a4eaaff4da2687b5a46a69aa49a31042dce5d975", baselineSha256: "adec5b5933cc70869be55efbabb613b555c890f0e755e01b13b28696e67c9b4a", launcherSha256, secureWrapperSha256: "f".repeat(64), executionEligible: false, candidateIssued: false, humanConfirmed: false, executionConsumed: false }));
