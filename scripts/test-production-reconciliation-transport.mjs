...
async function buildFreshTransportFixture() {
  ...
  const candidate = await buildAuthorizationV3FromPackage({ packageRoot, repositoryRoot, transportImplementationCommit: commit, transportLauncherSha256: launcherSha256, transportSha256, requiredConfirmationPhrase: confirmation });
  const authorizationPath = path.join(temp, "candidate-v3.json");
...
try {
  ...
  const finalPath = path.join(fixture.temp, "final-v2.json");
  ...
  for (const pgUser of ["postgres.aaaaaaaaaaaaaaaaaaaa", undefined, "", "postgres", "postgres.", "postgres..xcbnxzjlsvtgzixurcof", "postgres.xcbnxzjlsvtgzixurcof.extra"]) {
    ...
    const negativeFinal = path.join(negative.temp, "final-v2.json");
  ...
  const stale = path.join(fixture.temp, "candidate-v2.json");
  await writeFile(stale, '{"schemaVersion":"qa-production-reconciliation-execution-authorization-v2"}\n', "utf8");
  await assert.rejects(validateOnly({ ...core(fixture), authorizationPath: stale }), /AUTHORIZATION_V3_INVALID/);
