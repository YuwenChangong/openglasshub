...
import { executeOnce } from "./qa/r6-production-reconciliation-transport.mjs";
...
async function createPackage(temp) {
  ...
  return { packageRoot, manifestSha256: hash(await readFile(manifestPath)) };
}
...
try {
  ...
  const result = await withProductionDriftFixtureRuntime({ root, inputs, label: "transport-local", run: async (runtime) => {
    ...
    const authorization = { schemaVersion: AUTHORIZATION_VERSION, authorizationId: randomUUID(), executionTaskId: randomUUID(), authorizationSha256: "d".repeat(64), packageManifestSha256: packageFixture.manifestSha256, transportImplementationCommit: commit, transportLauncherSha256: "b".repeat(64), transportSha256: "c".repeat(64), targetIdentitySha256: targetIdentityHash(observed), canonicalMigrationSha256: CANONICAL_MIGRATION_SHA256, postflightSha256: POSTFLIGHT_SHA256, confirmationSha256: hash("local-transport-confirmation"), transportContractVersion: TRANSPORT_CONTRACT_VERSION, sqlClientCapability: "PSQL_NATIVE", sqlClientVersion: capability.version, sqlClientExecutablePath: capability.executablePath, sqlClientExecutableSha256: capability.executableSha256, attempts: 1, automaticRetry: 0, automaticRollback: 0 };
    const client = {
      async targetProbe(sql) { return { outcome: "TARGET_SUCCESS", observedProbeOutput: dockerPsql(runtime.container, sql, true).trim() }; },
    ...
    const execution = await executeOnce({ authorization, packageRoot: packageFixture.packageRoot, receiptRoot: path.join(temp, "receipts"), confirmationPhrase: "local-transport-confirmation", implementationCommit: commit, launcherSha256: "b".repeat(64), transportSha256: "c".repeat(64), sqlClientCapability: capability, client });
    const catalog = await captureCatalog(root, runtime.container);
    return { execution, comparison: compareFingerprint(inputs.expected, catalog.rows).counts };
  }});
  assert.equal(result.execution.classification, "R6_PRODUCTION_RECONCILIATION_EXECUTION_AND_POSTFLIGHT_COMPLETE");
  assert.equal(result.execution.postflightCount, 1); assert.deepEqual(result.comparison, { MATCH: 1133, MISSING_IN_PRODUCTION: 0, DIVERGENT_IN_PRODUCTION: 0, EXTRA_IN_PRODUCTION: 20, INSUFFICIENT_EVIDENCE: 0 });
