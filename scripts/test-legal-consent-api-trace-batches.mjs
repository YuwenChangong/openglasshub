...
const externalVideoOrderingBlocker = methodTraceFixture.releaseBlockingFindings.find((finding) => finding?.id === "EXTERNAL_VIDEO_UPLOAD_TARGET_AUTHORIZATION_ORDERING");
...
assert.equal(new Set(completeTraceIds).size, 46);
assert.equal(expectedMethodCount - new Set(completeTraceIds).size, 20);
...
assert.equal(fileTraceProgress["src/pages/api/forum/external-video-upload.ts"]?.status, "blocked");
assert.deepEqual(fileTraceProgress["src/pages/api/forum/external-video-upload.ts"]?.completedMethodIds, []);
assert.deepEqual(fileTraceProgress["src/pages/api/forum/external-video-upload.ts"]?.pendingMethodIds, ["src/pages/api/forum/external-video-upload.ts#POST"]);
assert.equal(fileTraceProgress["src/pages/api/forum/external-video-upload.ts"]?.blockerId, "EXTERNAL_VIDEO_UPLOAD_TARGET_AUTHORIZATION_ORDERING");
...
assert.equal(fileTraceProgress["src/pages/api/forum/comments.ts"]?.batch5Progress?.nextPendingMethodId, "src/pages/api/forum/external-video-upload.ts#POST");
...
assert.equal(batch5?.methodIds.find((id) => !completeTraceIds.includes(id)), "src/pages/api/forum/external-video-upload.ts#POST");
