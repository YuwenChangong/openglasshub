...
export const JOURNAL_STATES = new Set([
  ...
  "FAILED_ZERO_WRITES", "PARTIAL_POST_CREATED_COMMENT_FAILED", "COMPLETE_TWO_WRITES", "BLOCKED_UNEXPECTED_WRITE",
]);
...
function assertPrepared(prepared, { recovery = false, legacy = false } = {}) {
  ...
  if (prepared.creationEnabled !== expectedRecovery[0] || prepared.recoveryOnly !== expectedRecovery[1] || prepared.plannedPostCount !== 1 || prepared.plannedCommentCount !== 1 || prepared.networkRetryPolicy !== "zero") throw new Error("QA_CANARY_PREPARED_SCOPE_INVALID");
  if (legacy) {
    if (prepared.cleanupOrder !== "comment-then-post") throw new Error("QA_CANARY_PREPARED_SCOPE_INVALID");
  } else if (prepared.cleanupContract !== "none" || prepared.rollbackContract !== "none" || prepared.persistenceContract !== "retain-created-post-and-comment") {
    throw new Error("QA_CANARY_PREPARED_SCOPE_INVALID");
  }
}
...
export function createJournal({ runId, prepared, markers = createMarkers(runId), now: createdAt = now() }) {
  ...
  return { schemaVersion: JOURNAL_SCHEMA_VERSION, runId, approval: CANARY_APPROVAL, state: "PREPARED", preparedAt: createdAt, prepared, markers, attempts: { post: null, comment: null }, artifacts: { post: null, comment: null }, recovery: { candidateClassification: null, adopted: false }, history: [{ state: "PREPARED", at: createdAt }] };
}

export function validateJournal(journal) {
  if (!journal || ![2, JOURNAL_SCHEMA_VERSION].includes(journal.schemaVersion) || journal.approval !== CANARY_APPROVAL || !JOURNAL_STATES.has(journal.state)) throw new Error("QA_CANARY_JOURNAL_SCHEMA_INVALID");
  validateCanaryRunId(journal.runId); const legacy = journal.schemaVersion === 2; assertPrepared(journal.prepared, { recovery: journal.prepared?.recoveryOnly === true, legacy });
  if (!journal.markers?.post || !journal.markers?.comment || !journal.attempts || !journal.artifacts || !journal.recovery || (legacy && !journal.cleanup)) throw new Error("QA_CANARY_JOURNAL_SCOPE_INVALID");
  return journal;
}
...
async function submit(kind, adapter, store, journal) {
  ...
  try {
    ...
    const failedState = ambiguous
      ? (kind === "post" ? "POST_SUBMISSION_AMBIGUOUS" : "COMMENT_SUBMISSION_AMBIGUOUS")
      : (kind === "post" ? "FAILED_ZERO_WRITES" : "PARTIAL_POST_CREATED_COMMENT_FAILED");
    await transition(store, withAttempt, failedState, {
...
export async function executeMinimalCanary({ adapter, store, journal }) {
  ...
  return transition(store, withComment, "COMPLETE_TWO_WRITES");
}
