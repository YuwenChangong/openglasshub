...
const REQUIRED = [
  ...
  "pagesRequestCount", "deploymentRequestCount", "supabaseReadCount", "supabaseWriteCount", "productionMutationCount", "childStarted", "childExitCode", "outerClassification", "innerClassification", "success",
];
...
export function validateR6V3AuthCheckTerminal(value) {
  ...
  for (const key of ["worktreeValidationPassed", "evidenceRootFresh", "attestationFreshnessPassed", "credentialPromptReached", "otpPromptReached", "authenticationAttempted", "authenticationCompleted", "sessionCreated", "sessionValidated", "authenticatedCheckReached", "authenticatedCheckCompleted", "childStarted", "success"]) if (typeof value[key] !== "boolean") fail("R6_V3_AUTH_CHECK_TERMINAL_SCHEMA_INVALID");
  ...
  if (value.success) {
    if (value.outerClassification !== "R6_CURRENT_CANONICAL_V3_AUTH_CHECK_ONLY_OK" || value.innerClassification !== null || value.childExitCode !== 0 || !value.worktreeValidationPassed || !value.attestationFreshnessPassed || !value.authenticationCompleted || !value.sessionValidated || !value.authenticatedCheckCompleted) fail("R6_V3_AUTH_CHECK_TERMINAL_SUCCESS_CONTRADICTION");
  } else if (value.outerClassification !== "R6_CURRENT_CANONICAL_V3_AUTH_CHECK_ONLY_FAILED" || typeof value.innerClassification !== "string" || value.childExitCode === 0) fail("R6_V3_AUTH_CHECK_TERMINAL_FAILURE_CONTRADICTION");
