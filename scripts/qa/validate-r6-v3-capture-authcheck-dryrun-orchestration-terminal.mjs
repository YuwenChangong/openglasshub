...
export function validateR6V3CaptureAuthCheckDryRunOrchestrationTerminal(value) {
  ...
  if (value.schemaVersion !== R6_V3_CAPTURE_AUTHCHECK_DRYRUN_ORCHESTRATION_TERMINAL_VERSION || value.worktreeContract !== "current-canonical-production-v3" || !RUN_ID.test(String(value.runId)) || ![value.startedAt, value.completedAt].every((entry) => TIME.test(String(entry))) || containsSensitiveValue(value)) fail("R6_V3_CAPTURE_AUTHCHECK_DRYRUN_ORCHESTRATION_TERMINAL_SCHEMA_INVALID");
