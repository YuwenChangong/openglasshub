...
async function main() {
  ...
  try {
    ...
    astro = startAstro(ports.api + 1000, { CLOUDFLARE_INCLUDE_PROCESS_ENV: "true", SUPABASE_URL: status.API_URL, SUPABASE_ANON_KEY: status.ANON_KEY, PUBLIC_SUPABASE_URL: status.API_URL, PUBLIC_SUPABASE_ANON_KEY: status.ANON_KEY }); const base = `http://127.0.0.1:${ports.api + 1000}`; await waitFor(`${base}/about/`, astro); astro.backgroundOwned = true;
