...
export async function validateIdentityGuards({ options, env = process.env, now = Date.now(), attestationRoot } = {}) {
  ...
  const attestation = await validateDeploymentAttestation({ attestationPath: identities.attestationPath, expectedSha256: identities.attestationSha256, expectedCommit: identities.expectedDeployedCommit, now, root: attestationRoot });
  assertFreshAttestation(attestation, now);
  return { cfg, runnerCommit, attestation, attestationSha256: identities.attestationSha256 };
}
...
export async function main(argv = process.argv.slice(2), env = process.env, dependencies = {}) {
  ...
  const adapter = (dependencies.createAdapter ?? createProductionMinimalCanaryHttpAdapter)(credentials); const completed = await executeMinimalCanary({ adapter, store, journal: persisted }); console.log(JSON.stringify({ phase: "COMPLETE", ...safeSummary(completed), journalSha256: sha256(JSON.stringify(completed)) }, null, 2));
}
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) main().catch((error) => { const message = String(error?.message ?? "QA_CANARY_FAILED").replace(/eyJ[A-Za-z0-9._-]+/g, "[redacted-jwt]"); console.error(`QA_CANARY_FAILED: ${message}`); process.exitCode = 1; });
