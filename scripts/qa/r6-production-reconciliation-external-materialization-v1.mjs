...
export async function loadExternalExecutionMaterializationReadyV1({ externalRoot, readyInventoryPath }) {
  ...
  if (!value || Object.keys(value).length !== readyKeys.length || Object.keys(value).some(key => !readyKeys.includes(key)) || value.schemaVersion !== "r6-production-reconciliation-external-materialization-inventory-v1" || value.state !== "READY" || value.ready !== true || !/^[a-f0-9]{40}$/.test(String(value.sourceCommit)) || value.expectedProjectRef !== "xcbnxzjlsvtgzixurcof" || Number.isNaN(Date.parse(String(value.issuedAtUtc))) || ["canonicalLauncherTemplateSha256","canonicalSecureWrapperSourceSha256","secureWrapperSha256","credentialBrokerSha256","materializationSha256","launcherBindingSha256","renderedLauncherObservedSha256","executeApprovalSha256","finalExecutionAuthoritySha256"].some(key => !HASH.test(String(value[key])))) fail("R6_PRODUCTION_RECONCILIATION_EXTERNAL_READY_INVALID");
