...
import { CANONICAL_DPAPI_CREDENTIAL_BROKER_SOURCE_RELATIVE_PATH, loadCanonicalSecureWrapperSourceAuthority } from "../lib/r6-canonical-secure-wrapper-source-authority.mjs";
...
const readyKeys = ["schemaVersion","sourceCommit","state","ready","canonicalLauncherTemplateSha256","canonicalSecureWrapperSourceSha256","secureWrapperRelativePath","secureWrapperSha256","credentialBrokerRelativePath","credentialBrokerSha256","materializationRelativePath","materializationSha256","launcherBindingRelativePath","launcherBindingSha256","renderedLauncherRelativePath","renderedLauncherObservedSha256","executeApprovalSha256","finalExecutionAuthoritySha256","expectedProjectRef","connectionMode","pgHost","pgPort","pgDatabase","pgUser","issuedAtUtc"];
...
export async function materializeExternalSecureWrapperV1({ repositoryRoot, externalRoot }) {
  ...
  const credentialBrokerPath = path.join(root, "r6-production-dpapi-credential-broker.ps1");
  await copyFile(source.canonicalSecureWrapperSourcePath, wrapperPath, 0);
  await copyFile(path.join(repositoryRoot, CANONICAL_DPAPI_CREDENTIAL_BROKER_SOURCE_RELATIVE_PATH), credentialBrokerPath, 0);
  const observedBytes = await readFile(wrapperPath);
  const credentialBrokerBytes = await readFile(credentialBrokerPath);
  ...
  const inventory = { schemaVersion: "r6-production-reconciliation-external-root-creation-receipt-v1", externalRoot: root, state: "PREPARING", canonicalSecureWrapperSourceSha256: source.canonicalSecureWrapperSourceSha256, secureWrapperRelativePath: path.basename(wrapperPath), secureWrapperSha256, credentialBrokerRelativePath: path.basename(credentialBrokerPath), credentialBrokerSha256: hash(credentialBrokerBytes), ready: false };
  let handle;
  try { handle = await open(inventoryPath, "wx", 0o600); await handle.writeFile(`${JSON.stringify(inventory)}\n`); } finally { await handle?.close(); }
  return Object.freeze({ externalRoot: root, wrapperPath, credentialBrokerPath, inventoryPath, canonicalSecureWrapperSourceSha256: source.canonicalSecureWrapperSourceSha256, secureWrapperSha256, credentialBrokerSha256: hash(credentialBrokerBytes), ready: false });
}
...
export async function prepareExternalExecutionMaterializationV1(input) {
  ...
  const files = await Promise.all([base.wrapperPath, base.credentialBrokerPath, materializationPath, bindingPath, launcherPath].map(file => readFile(file)));
  const route = prepared.finalExecutionAuthority.routeAuthority;
  const ready = { schemaVersion: "r6-production-reconciliation-external-materialization-inventory-v1", sourceCommit: prepared.finalExecutionAuthority.sourceCommit, state: "READY", ready: true, canonicalLauncherTemplateSha256: prepared.finalExecutionAuthority.canonicalLauncherTemplateSha256, canonicalSecureWrapperSourceSha256: base.canonicalSecureWrapperSourceSha256, secureWrapperRelativePath: path.basename(base.wrapperPath), secureWrapperSha256: hash(files[0]), credentialBrokerRelativePath: path.basename(base.credentialBrokerPath), credentialBrokerSha256: hash(files[1]), materializationRelativePath: path.basename(materializationPath), materializationSha256: hash(files[2]), launcherBindingRelativePath: path.basename(bindingPath), launcherBindingSha256: hash(files[3]), renderedLauncherRelativePath: path.basename(launcherPath), renderedLauncherObservedSha256: hash(files[4]), executeApprovalSha256: prepared.finalExecutionAuthority.executeApprovalSha256, finalExecutionAuthoritySha256: hash(Buffer.from(JSON.stringify(prepared.finalExecutionAuthority))), expectedProjectRef: prepared.finalExecutionAuthority.expectedProjectRef, connectionMode: route.connectionMode, pgHost: route.pgHost, pgPort: route.pgPort, pgDatabase: route.pgDatabase, pgUser: route.pgUser, issuedAtUtc: new Date().toISOString() };
...
export async function loadExternalExecutionMaterializationReadyV1({ externalRoot, readyInventoryPath }) {
  ...
  if (!value || Object.keys(value).length !== readyKeys.length || Object.keys(value).some(key => !readyKeys.includes(key)) || value.schemaVersion !== "r6-production-reconciliation-external-materialization-inventory-v1" || value.state !== "READY" || value.ready !== true || !/^[a-f0-9]{40}$/.test(String(value.sourceCommit)) || value.expectedProjectRef !== "xcbnxzjlsvtgzixurcof" || value.connectionMode !== "SHARED_POOLER_SESSION" || value.pgHost !== "aws-1-ap-northeast-1.pooler.supabase.com" || value.pgPort !== "5432" || value.pgDatabase !== "postgres" || value.pgUser !== "postgres.xcbnxzjlsvtgzixurcof" || Number.isNaN(Date.parse(String(value.issuedAtUtc))) || ["canonicalLauncherTemplateSha256","canonicalSecureWrapperSourceSha256","secureWrapperSha256","credentialBrokerSha256","materializationSha256","launcherBindingSha256","renderedLauncherObservedSha256","executeApprovalSha256","finalExecutionAuthoritySha256"].some(key => !HASH.test(String(value[key])))) fail("R6_PRODUCTION_RECONCILIATION_EXTERNAL_READY_INVALID");
  const paths = { wrapper: contained(root, value.secureWrapperRelativePath), credentialBroker: contained(root, value.credentialBrokerRelativePath), materialization: contained(root, value.materializationRelativePath), binding: contained(root, value.launcherBindingRelativePath), launcher: contained(root, value.renderedLauncherRelativePath) };
  ...
  if (observed.wrapper !== value.secureWrapperSha256 || observed.credentialBroker !== value.credentialBrokerSha256 || observed.materialization !== value.materializationSha256 || observed.binding !== value.launcherBindingSha256 || observed.launcher !== value.renderedLauncherObservedSha256) fail("R6_PRODUCTION_RECONCILIATION_EXTERNAL_READY_BINDING_FAILED");
  ...
  if (binding.materializationPath !== paths.materialization || binding.launcherPath !== paths.launcher
    ...
    || path.resolve(launcherConfig.repositoryRoot ?? "") !== path.resolve(process.cwd())
    || ["connectionMode","pgHost","pgPort","pgDatabase","pgUser"].some(key => value[key] !== materialization[key] || value[key] !== launcherConfig.routeAuthority?.[key])) fail("R6_PRODUCTION_RECONCILIATION_EXTERNAL_READY_LINEAGE_INVALID");
