...
for (const key of ["runId", "operatorRoot", "manifestPath", "manifestSha256", "launcherPath", "evidenceRoot", "executionWorktree", "wrapperPath", "wrapperSha256"]) if (typeof config[key] !== "string" || !config[key]) fail("R6_OPERATOR_LAUNCH_RENDER_CONFIG_INVALID");
if (!/^qa-canary-[0-9a-f-]{36}$/.test(config.runId) || !/^[a-f0-9]{64}$/.test(config.wrapperSha256) || !/^[a-f0-9]{64}$/.test(config.manifestSha256)) fail("R6_OPERATOR_LAUNCH_RENDER_CONFIG_INVALID");
