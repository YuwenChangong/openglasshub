...
export const R6_V3_DRY_RUN_TERMINAL_VERSION = "r6-v3-dry-run-terminal-result-v2";
...
export function validateR6V3DryRunTerminal(value) {
  ...
  if (![LEGACY_VERSION, R6_V3_DRY_RUN_TERMINAL_VERSION].includes(value.schemaVersion) || !RUN_ID.test(String(value.runId)) || ![value.startedAt, value.completedAt].every((entry) => TIME.test(String(entry))) || SENSITIVE.test(JSON.stringify(value))) fail("R6_V3_DRY_RUN_TERMINAL_SCHEMA_INVALID");
  ...
  if (value.schemaVersion === R6_V3_DRY_RUN_TERMINAL_VERSION) {
  ...
  if (value.success) {
    if (value.outerClassification !== "R6_CURRENT_CANONICAL_V3_DRY_RUN_ONLY_READY" || value.innerClassification !== null || value.failureStage !== "complete" || !value.captureProvenancePassed || !value.authProvenancePassed || !value.attestationFreshnessPassed || !value.childStarted || !value.childCompleted || value.childTimedOut || value.childExitCode !== 0 || value.remainingValidityMs < value.minimumRequiredValidityMs || (value.schemaVersion === R6_V3_DRY_RUN_TERMINAL_VERSION && (value.targetBinding === null || value.targetBindingPath === null || value.targetBindingSha256 === null))) fail("R6_V3_DRY_RUN_TERMINAL_SUCCESS_CONTRADICTION");
