...
export const R6_V4_DRY_RUN_TERMINAL_VERSION = "r6-v4-dry-run-terminal-result-v4";
...
const V4_REQUIRED = ["unexpectedMutationCount", "finalAuthorizationCreated"];
...
function containsSensitiveValue(value) {
  const classifications = new Set(["schemaVersion", "outerClassification", "innerClassification", "failureStage", "stdoutClassification", "stderrClassification", "targetResolutionFailureCategory", "targetResultCountClass", "targetEligibleState"]);
  return Object.entries(value).some(([key, entry]) => typeof entry === "string" && !classifications.has(key) && SENSITIVE.test(entry));
}
...
function requiredFor(version) {
  ...
  if (version === R6_V4_DRY_RUN_TERMINAL_VERSION) return [...BASE_REQUIRED, ...TARGET_DIAGNOSTIC_REQUIRED, ...V4_REQUIRED];
  return null;
}

function assertTargetDiagnostics(value) {
  for (const key of ["authenticationCompleted", "targetResolutionStarted", "targetResolutionCompleted", "targetResolutionSucceeded", "canonicalTargetResolved", "canonicalCircleIdResolved", "canonicalCircleSlugResolved", "targetBindingArtifactPresent", "targetBindingValidationPassed", "targetBindingCreated", "targetBindingHashCreated", "targetBoundExecutionPlanHashCreated"]) if (typeof value[key] !== "boolean") fail("R6_V3_DRY_RUN_TERMINAL_TARGET_DIAGNOSTIC_INVALID");
...
export function validateR6V3DryRunTerminal(value) {
  ...
  if (!RUN_ID.test(String(value.runId)) || ![value.startedAt, value.completedAt].every((entry) => TIME.test(String(entry))) || containsSensitiveValue(value)) fail("R6_V3_DRY_RUN_TERMINAL_SCHEMA_INVALID");
  ...
  if (value.schemaVersion === R6_V4_DRY_RUN_TERMINAL_VERSION && typeof value.finalAuthorizationCreated !== "boolean") fail("R6_V3_DRY_RUN_TERMINAL_SCHEMA_INVALID");
  for (const key of ["minimumRequiredValidityMs", "remainingValidityMs", "childExitCode", "plannedMutationCount", "actualMutationCount", "supabaseWriteCount", "productionMutationCount", "retryCount", ...(value.schemaVersion === R6_V4_DRY_RUN_TERMINAL_VERSION ? ["unexpectedMutationCount"] : [])]) if (!Number.isInteger(value[key])) fail("R6_V3_DRY_RUN_TERMINAL_SCHEMA_INVALID");
  if (!["NOT_CREATED_OR_UNCONFIRMED", "PENDING", "CONSUMED"].includes(value.receiptState)) fail("R6_V3_DRY_RUN_TERMINAL_SCHEMA_INVALID");
  if (value.actualMutationCount !== 0 || value.supabaseWriteCount !== 0 || value.productionMutationCount !== 0 || value.retryCount !== 0 || value.plannedMutationCount !== 2 || (value.schemaVersion === R6_V4_DRY_RUN_TERMINAL_VERSION && value.unexpectedMutationCount !== 0)) fail("R6_V3_DRY_RUN_TERMINAL_SAFETY_INVALID");
  if (!COMMIT.test(String(value.executionCommit)) || (value.receiptRunnerCommit !== null && !COMMIT.test(String(value.receiptRunnerCommit))) || (value.expectedToolingCommit !== null && !COMMIT.test(String(value.expectedToolingCommit))) || value.adapterReached || value.journalCreated || (value.schemaVersion === R6_V4_DRY_RUN_TERMINAL_VERSION && value.finalAuthorizationCreated) || value.canaryChildStarted !== value.childStarted || (value.childCompleted && !value.childStarted) || (value.childTimedOut && !value.childCompleted)) fail("R6_V3_DRY_RUN_TERMINAL_SAFETY_INVALID");
  ...
  if ([R6_V3_DRY_RUN_TERMINAL_VERSION, R6_V4_DRY_RUN_TERMINAL_VERSION].includes(value.schemaVersion)) assertTargetDiagnostics(value);
