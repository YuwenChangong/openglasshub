...
function assertNoSensitive(value, key = "") {
  if (SENSITIVE_KEY.test(key) || (typeof value === "string" && SENSITIVE_VALUE.test(value))) fail("SECRET_OR_DSN_REJECTED");
...
function validateProduction(production) {
  ...
  if (production.writes === 0) {
    ...
    return { writes: 0, authorizationEvidence: null };
  }
  ...
  return { writes: production.writes, authorizationEvidence: { authorizationId, authorizedAt: normalizedAuthorizedAt } };
}
