...
export function validateP10LocalEvidence(evidence) {
  if (evidence?.productionShaped !== "PASS" || evidence?.alreadyCanonical !== "PASS" || evidence?.rollback !== "PASS" || evidence?.receiptHistoryAbsent !== "PASS" || evidence?.receiptHistoryPresent !== "PASS" || evidence?.productionConnections !== 0 || evidence?.productionMutations !== 0 || evidence?.cleanup !== "PASS") {
...
export async function runP10ForwardReconciliationLocal() {
  ...
  const evidence = { productionShaped: "FAIL", alreadyCanonical: "FAIL", rollback: "FAIL", receiptHistoryAbsent: "FAIL", receiptHistoryPresent: "FAIL", productionConnections: 0, productionMutations: 0, cleanup: "FAIL" };
  try {
    ...
    await command(PSQL, ["-X", "-q", "-v", "ON_ERROR_STOP=1", "-c", "INSERT INTO supabase_migrations.schema_migrations (version, name) VALUES ('20260902042807', 'forward_reconcile_devices');"], { env: { ...process.env, ...connection.pgEnv } });
