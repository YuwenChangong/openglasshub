...
function noWriteResult({ startedAt, productionRunId, parentDryRunRunId, executionTerminalPath, executionTerminalSha256, liveReceiptPath, liveReceiptSha256, journalPath, journalSha256, mutationPlanHash, targetBinding, executionCommit, toolingCommit }) {
  return { schemaVersion: FINAL_POSTFLIGHT_VERSION, startedAt, completedAt: isoNow(), outerClassification: "R6_FINAL_CANARY_READ_ONLY_POSTFLIGHT_FAILED", innerClassification: null, failureStage: "POSTFLIGHT", success: false, productionRunId, parentDryRunRunId, executionTerminalPath, executionTerminalSha256, executionTerminalValidated: false, liveReceiptPath, liveReceiptSha256, liveReceiptState: "PENDING", receiptVerified: false, journalPath, journalSha256, journalVerified: false, mutationPlanHash, targetBinding, approvedMutationCount: 2, executionActualMutationCount: 0, verifiedMutationCount: 0, unexpectedMutationCount: 0, duplicateExecutionCount: 0, executionCommit, toolingCommit, commitBindingPassed: false, supabaseReadCount: 0, supabaseWriteCount: 0, productionMutationCountDuringPostflight: 0 };
}
export async function runReadOnlyPostflight({ executionTerminalPath, executionTerminalSha256, receiptPath, receiptSha256, registryRoot, journalRoot, outputPath, readVerifier, now = isoNow } = {}) {
  ...
  try {
    ...
    productionRunId = terminal.productionRunId; parentDryRunRunId = terminal.parentDryRunRunId; executionCommit = terminal.executionCommit; toolingCommit = terminal.toolingCommit; mutationPlanHash = terminal.mutationPlanHash; targetBinding = terminal.targetBinding; journalPath = terminal.journalPath; journalSha256 = terminal.journalSha256;
    ...
    if (sha256(journalRaw) !== journalSha256 || journal.runId !== productionRunId || journal.state !== "COMPLETE_TWO_WRITES" || journal.prepared?.runnerCommit !== executionCommit || journal.prepared?.cleanupContract !== "none" || journal.prepared?.rollbackContract !== "none") fail("R6_FINAL_POSTFLIGHT_JOURNAL_INVALID");
