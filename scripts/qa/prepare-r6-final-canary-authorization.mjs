...
const targetBinding = validateCanonicalCanaryTargetBinding(dry.value.targetBinding, { baseMutationPlanSchema: plan.schemaVersion, baseMutationPlanHash: plan.planSha256, executionCommit: dry.value.executionCommit, toolingCommit: dry.value.expectedToolingCommit });
if (JSON.stringify(receipt.value.targetBinding) !== JSON.stringify(targetBinding) || JSON.stringify(orchestration.value.targetBinding) !== JSON.stringify(targetBinding)) fail("R6_FINAL_DRY_RUN_TARGET_BINDING_INELIGIBLE");
const authorization = { schemaVersion: FINAL_AUTHORIZATION_VERSION, dryRunRunId: dry.value.runId, dryRunReceiptPath: path.resolve(receiptPath), dryRunReceiptSha256: sha256(receipt.raw), dryRunTerminalPath: path.resolve(dryTerminalPath), dryRunTerminalSha256: sha256(dry.raw), dryRunOrchestrationTerminalPath: path.resolve(orchestrationPath), dryRunOrchestrationTerminalSha256: sha256(orchestration.raw), executionCommit: dry.value.executionCommit, toolingCommit: dry.value.expectedToolingCommit, plan, targetBinding, plannedMutationCount: 2, actualMutationCount: 0, supabaseWriteCount: 0, productionMutationCount: 0, retryCount: 0, successClassification: dry.value.outerClassification };
