...
import { bindConfirmationPhraseToCandidateV1, issueTestOnlyConfirmationPhraseV1, loadConfirmationPhraseIssuanceV1 } from "./r6-production-reconciliation-confirmation-phrase-v1.mjs";
...
export async function issueAttestedCandidateV3({ candidateRoot, packageRoot, repositoryRoot, transportImplementationCommit, transportLauncherSha256, transportSha256, confirmationIssuancePath, requiredConfirmationPhrase, now = new Date().toISOString(), testOnly = false, testAuthorityRoot } = {}) {
  if (!confirmationIssuancePath) {
    if (!testOnly || typeof requiredConfirmationPhrase !== "string") fail("R6_PRODUCTION_RECONCILIATION_CONFIRMATION_ISSUANCE_REQUIRED");
    const issued = await issueTestOnlyConfirmationPhraseV1({ repositoryRoot, sourceCommit: transportImplementationCommit, confirmationPhrase: requiredConfirmationPhrase, now, testAuthorityRoot });
    confirmationIssuancePath = issued.receipt.path;
  }
  const issuance = await loadConfirmationPhraseIssuanceV1({ repositoryRoot, issuancePath: confirmationIssuancePath, sourceCommit: transportImplementationCommit, testOnly, testAuthorityRoot });
  const candidate = await buildAuthorizationV4FromPackage({ packageRoot, repositoryRoot, transportImplementationCommit, transportLauncherSha256, transportSha256, requiredConfirmationSha256: issuance.value.confirmationPhraseSha256 });
  await bindConfirmationPhraseToCandidateV1({ repositoryRoot, issuance, candidate, testOnly, testAuthorityRoot, now });
