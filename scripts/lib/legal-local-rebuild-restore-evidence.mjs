...
const FIELDS = new Set(["schemaVersion", "taskId", "implementationCommit", "targetBindingSha256", "bootstrapFingerprintSha256", "preMigrationFingerprintSha256", "rebuiltFingerprintSha256", "destroyedContainerIdentityHash", "rebuiltContainerIdentityHash", "bootstrappedAtUtc", "destroyedAtUtc", "rebuiltAtUtc", "destroyObserved", "rebuildObserved", "restoreSmoke"]);
...
export function validateLegalLocalRebuildRestoreEvidence(evidence, { targetBinding, now = Date.now() } = {}) {
  ...
  if (Object.keys(evidence).some((field) => !FIELDS.has(field))) fail("R6_LOCAL_REBUILD_RESTORE_UNKNOWN_FIELD");
