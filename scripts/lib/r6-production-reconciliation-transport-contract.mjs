...
export function validateAuthorizationV2(value, { implementationCommit, launcherSha256, transportSha256, sqlClientCapability = null } = {}) {
  ...
  const hashFields = ["transportLauncherSha256", "transportSha256", "targetIdentitySha256", "canonicalMigrationSha256", "postflightSha256", "confirmationSha256", "authorizationSha256", "packageManifestSha256", "sqlClientExecutableSha256"];
  if (hashFields.some((key) => !HASH.test(String(value[key] ?? ""))) || !COMMIT.test(value.transportImplementationCommit) || typeof value.sqlClientVersion !== "string" || !path.isAbsolute(String(value.sqlClientExecutablePath ?? "")) || value.canonicalMigrationSha256 !== CANONICAL_MIGRATION_SHA256 || value.postflightSha256 !== POSTFLIGHT_SHA256 || value.transportContractVersion !== TRANSPORT_CONTRACT_VERSION || value.sqlClientCapability !== "PSQL_NATIVE" || value.attempts !== 1 || value.automaticRetry !== 0 || value.automaticRollback !== 0) {
...
export async function loadBoundExecutionPackage({ packageRoot, expectedPackageSha256 = null }) {
  ...
  if (expectedPackageSha256 && sha256(manifestBytes) !== expectedPackageSha256) fail("R6_PRODUCTION_RECONCILIATION_PACKAGE_MANIFEST_MISMATCH");
  let manifest;
  ...
  return Object.freeze({ manifest, manifestPath, manifestSha256: sha256(manifestBytes), migration, postflight, targetProbe });
}
