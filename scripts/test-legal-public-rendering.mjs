...
await new Promise((resolve, reject) => {
  const build = spawn(process.execPath, [path.join(root, "scripts", "build-workers.mjs")], {
    cwd: root, env: { ...process.env }, stdio: ["ignore", "ignore", "pipe"], windowsHide: true,
  });
  let errors = "";
  build.stderr.on("data", (chunk) => { errors = (errors + chunk).slice(-2000); });
  build.on("error", reject);
  build.on("close", (code) => code === 0 ? resolve() : reject(new Error(`local build exited ${code}: ${errors}`)));
});
for (const route of ["terms", "privacy", "community-guidelines"]) {
  const html = await readFile(path.join(root, "dist", "client", route, "index.html"), "utf8");
