...
async function main() {
  ...
  assert(!productsIndex.includes("products-compare-search"), "Products index should not include a second compare search.");
  assert(productsIndex.includes("查看产品"), "Products index should retain a concise detail CTA.");
  ...
  assert(!productsIndex.includes("product-visual__pill"), "Products index should not surface visual corner pills in placeholders.");
  assert(productsIndex.includes("product-card__footer"), "Products index should place actions in the product card footer.");
  ...
  assert(!brandPage.includes("/devices/"), "Brand page should no longer link into /devices/.");
  ...
  assert(!visual.includes("product-visual__pill"), "Product visual should not render top-right category pills.");
  assert(navigation.includes('label: "产品"'), "Main navigation should keep the products entry.");
  assert(!navigation.includes('label: "设备库"'), "Main navigation should no longer include the device library entry.");
  ...
  const stripTypesCheck = runNodeWithStripTypes(`
    const payload = {
      ...
      hasSingleSearch: ${productsIndex.includes('id="products-search"') && !productsIndex.includes("products-compare-search")},
    };
  ...
  assert(parsed.hasSingleSearch, "Strip-types sanity check should confirm the single-search layout.");
