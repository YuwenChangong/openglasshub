...
function ConvertTo-BindingFixtureJson([object]$Value) {
  return ($Value | ConvertTo-Json -Depth 32 -Compress)
}

function Require-DryRunLifecycleMirror([object]$OrchestrationTerminal, [object]$DryRunTerminal) {
  $pairs = @(
    @('dryRunExecutionCommit','executionCommit'), @('dryRunReceiptRunnerCommit','receiptRunnerCommit'), @('dryRunExpectedToolingCommit','expectedToolingCommit'),
    @('dryRunPlannedMutationCount','plannedMutationCount'), @('dryRunActualMutationCount','actualMutationCount'), @('dryRunReservationAttempted','reservationAttempted'),
    @('dryRunReservationCompleted','reservationCompleted'), @('dryRunReceiptCreated','receiptCreated'), @('dryRunReceiptState','receiptState'),
    @('dryRunExecutorStarted','childStarted'), @('dryRunCanaryChildStarted','canaryChildStarted'), @('dryRunExecutorCompleted','childCompleted'),
    @('dryRunExecutorTimedOut','childTimedOut'), @('dryRunJournalCreated','journalCreated'), @('dryRunFinalAuthorizationCreated','finalAuthorizationCreated'),
    @('dryRunUnexpectedMutationCount','unexpectedMutationCount'), @('dryRunSupabaseWriteCount','supabaseWriteCount')
  )
  foreach ($pair in $pairs) {
    Require ($OrchestrationTerminal.($pair[0]) -eq $DryRunTerminal.($pair[1])) ('R6_CURRENT_CANONICAL_V3_DRY_RUN_LIFECYCLE_MIRROR_FAILED:' + $pair[0])
  }
}
...
function Invoke-ThreeStageOrchestrationFixture([string]$ReservationFailure = '', [string]$AuthFixtureKind = '', [string]$PreToolingFailure = '', [string]$TargetResolutionFailure = '', [string]$PostReservationFailure = '', [string]$BindingMutation = '', [string]$ChildScenario = '', [switch]$UseRealTargetResolver, [switch]$PreexistingSecret) {
  ...
  try {
    ...
    if ($PreexistingSecret) {
      $terminal = Get-Content -LiteralPath $terminalPath -Raw | ConvertFrom-Json
      Require ($exitCode -eq 1 -and $LASTEXITCODE -eq 0 -and $validated.Count -eq 1 -and $validated[0].ToString().Trim() -eq 'R6_V3_CAPTURE_AUTHCHECK_DRYRUN_ORCHESTRATION_TERMINAL_OK' -and $terminal.outerClassification -eq 'R6_CURRENT_CANONICAL_V3_DRY_RUN_ORCHESTRATION_PRE_FLIGHT_UNSAFE' -and $terminal.innerClassification -eq 'R6_PREEXISTING_SECRET_ENV_DENIED' -and $terminal.failureStage -eq 'preflight_secret_environment' -and -not [bool]$terminal.captureStarted -and -not [bool]$terminal.authCheckStarted -and -not [bool]$terminal.dryRunStarted -and -not [bool]$terminal.dryRunReservationAttempted -and -not [bool]$terminal.dryRunReceiptCreated -and -not [bool]$terminal.dryRunExecutorStarted -and [int]$terminal.productionMutationCount -eq 0 -and [int]$terminal.supabaseWriteCount -eq 0) 'R6_CURRENT_CANONICAL_V3_THREE_STAGE_PREEXISTING_SECRET_FAIL_CLOSED_FAILED'
      return
    }
    if (-not [string]::IsNullOrWhiteSpace($BindingMutation)) {
      $terminal = Get-Content -LiteralPath $terminalPath -Raw | ConvertFrom-Json
      $dryTerminal = Get-Content -LiteralPath $dryTerminalPath -Raw | ConvertFrom-Json
      Require ($exitCode -eq 1 -and $LASTEXITCODE -eq 0 -and $validated.Count -eq 1 -and $validated[0].ToString().Trim() -eq 'R6_V3_CAPTURE_AUTHCHECK_DRYRUN_ORCHESTRATION_TERMINAL_OK' -and -not [bool]$terminal.targetResolutionStarted -and -not [bool]$terminal.targetResolutionSucceeded -and $null -eq $terminal.targetBinding -and $null -eq $terminal.targetBindingPath -and $null -eq $terminal.targetBindingSha256 -and [bool]$dryTerminal.targetResolutionSucceeded) ('R6_CURRENT_CANONICAL_V3_THREE_STAGE_TARGET_BINDING_TAMPER_REJECTION_FAILED:' + $BindingMutation)
      return
    }
    ...
    $terminal = Get-Content -LiteralPath $terminalPath -Raw | ConvertFrom-Json
    if ((Test-Path -LiteralPath $dryTerminalPath -PathType Leaf) -and [string]::IsNullOrWhiteSpace($BindingMutation)) {
      $mirroredDryTerminal = Get-Content -LiteralPath $dryTerminalPath -Raw | ConvertFrom-Json
      Require-DryRunLifecycleMirror $terminal $mirroredDryTerminal
    }
    if (-not [string]::IsNullOrWhiteSpace($AuthFixtureKind)) {
    ...
    } elseif (-not [string]::IsNullOrWhiteSpace($TargetResolutionFailure)) {
      $dryTerminal = Get-Content -LiteralPath $dryTerminalPath -Raw | ConvertFrom-Json
      $targetDiagnostic = @{ 'QA_CANARY_TARGET_REQUESTED_SLUG_INVALID'=@('TARGET_INPUT_INVALID','UNKNOWN','UNKNOWN'); 'QA_CANARY_TARGET_NOT_FOUND'=@('TARGET_NOT_FOUND','ZERO','UNKNOWN'); 'QA_CANARY_TARGET_AMBIGUOUS'=@('TARGET_NON_UNIQUE','MULTIPLE','UNKNOWN'); 'QA_CANARY_TARGET_INELIGIBLE'=@('TARGET_INELIGIBLE','ONE','INELIGIBLE'); 'QA_CANARY_TARGET_CIRCLE_ID_MISSING'=@('TARGET_RESOLUTION_INCOMPLETE','ONE','UNKNOWN'); 'QA_CANARY_TARGET_BINDING_OUTPUT_EXISTS'=@('BINDING_ARTIFACT_PRESENT','UNKNOWN','UNKNOWN'); 'QA_CANARY_TARGET_BINDING_MISSING'=@('BINDING_ARTIFACT_MISSING','UNKNOWN','UNKNOWN'); 'QA_CANARY_TARGET_BINDING_INVALID'=@('BINDING_ARTIFACT_INVALID','ONE','UNKNOWN'); 'QA_CANARY_TARGET_RESOLUTION_OUTPUT_INVALID'=@('RESOLVER_OUTPUT_INVALID','UNKNOWN','UNKNOWN'); 'QA_CANARY_TARGET_RESOLUTION_PROCESS_FAILED'=@('RESOLVER_PROCESS_FAILURE','UNKNOWN','UNKNOWN'); 'QA_CANARY_TARGET_UNRECOGNIZED_FAILURE'=@('UNKNOWN_TARGET_RESOLUTION_FAILURE','UNKNOWN','UNKNOWN') }
      $expectedDiagnostic = $targetDiagnostic[$TargetResolutionFailure]
      Require ($null -ne $expectedDiagnostic) 'R6_CURRENT_CANONICAL_V3_THREE_STAGE_TARGET_DIAGNOSTIC_FIXTURE_INVALID'
      Require ($exitCode -eq 1 -and $terminal.outerClassification -eq 'R6_CURRENT_CANONICAL_V3_DRY_RUN_ORCHESTRATION_DRY_RUN_FAILED' -and $terminal.innerClassification -eq $TargetResolutionFailure -and $terminal.failureStage -eq 'TARGET_RESOLUTION' -and [bool]$terminal.captureSuccess -and [bool]$terminal.authCheckSuccess -and [bool]$terminal.dryRunStarted -and [bool]$terminal.dryRunCompleted -and -not [bool]$terminal.dryRunSuccess -and [bool]$terminal.dryRunAuthenticationCompleted -and [bool]$terminal.targetResolutionStarted -and [bool]$terminal.targetResolutionCompleted -and -not [bool]$terminal.targetResolutionSucceeded -and $terminal.targetResolutionFailureCategory -eq $expectedDiagnostic[0] -and $terminal.targetResultCountClass -eq $expectedDiagnostic[1] -and $terminal.targetEligibleState -eq $expectedDiagnostic[2] -and -not [bool]$terminal.canonicalCircleIdResolved -and -not [bool]$terminal.canonicalCircleSlugResolved -and $null -eq $terminal.dryRunReceiptRunnerCommit -and $null -eq $terminal.dryRunExpectedToolingCommit -and $null -eq $terminal.targetBinding -and $null -eq $terminal.targetBindingPath -and $null -eq $terminal.targetBindingSha256 -and $dryTerminal.innerClassification -eq $TargetResolutionFailure -and $dryTerminal.failureStage -eq 'TARGET_RESOLUTION' -and [bool]$dryTerminal.authenticationCompleted -and [bool]$dryTerminal.targetResolutionStarted -and [bool]$dryTerminal.targetResolutionCompleted -and -not [bool]$dryTerminal.targetResolutionSucceeded -and $dryTerminal.targetResolutionFailureCategory -eq $expectedDiagnostic[0] -and $dryTerminal.targetResultCountClass -eq $expectedDiagnostic[1] -and $dryTerminal.targetEligibleState -eq $expectedDiagnostic[2] -and -not [bool]$dryTerminal.reservationAttempted -and -not [bool]$dryTerminal.receiptCreated -and $dryTerminal.receiptState -eq 'NOT_CREATED_OR_UNCONFIRMED' -and $null -eq $dryTerminal.receiptRunnerCommit -and $null -eq $dryTerminal.expectedToolingCommit -and $null -eq $dryTerminal.targetBinding -and $null -eq $dryTerminal.targetBindingPath -and $null -eq $dryTerminal.targetBindingSha256 -and -not [bool]$dryTerminal.childStarted -and -not [bool]$dryTerminal.canaryChildStarted -and -not [bool]$dryTerminal.adapterReached -and -not [bool]$dryTerminal.journalCreated -and [int]$dryTerminal.actualMutationCount -eq 0 -and [int]$dryTerminal.supabaseWriteCount -eq 0 -and [int]$dryTerminal.productionMutationCount -eq 0) 'R6_CURRENT_CANONICAL_V3_THREE_STAGE_TARGET_RESOLUTION_FAILURE_CONTRACT_FAILED'
      ...
      Require ($exitCode -eq 1 -and $terminal.outerClassification -eq 'R6_CURRENT_CANONICAL_V3_DRY_RUN_ORCHESTRATION_DRY_RUN_FAILED' -and $terminal.innerClassification -eq $ReservationFailure -and $terminal.failureStage -eq 'RUN_ID_RESERVATION' -and [bool]$terminal.captureSuccess -and [bool]$terminal.authCheckSuccess -and -not [bool]$terminal.dryRunSuccess -and $dryTerminal.innerClassification -eq $ReservationFailure -and $dryTerminal.failureStage -eq 'RUN_ID_RESERVATION' -and [bool]$dryTerminal.targetResolutionSucceeded -and -not [bool]$dryTerminal.canaryChildStarted -and -not [bool]$dryTerminal.receiptCreated -and [int]$dryTerminal.actualMutationCount -eq 0 -and $terminal.targetBindingPath -eq $dryTerminal.targetBindingPath -and $terminal.targetBindingSha256 -eq $dryTerminal.targetBindingSha256 -and (ConvertTo-BindingFixtureJson $terminal.targetBinding) -ceq (ConvertTo-BindingFixtureJson $dryTerminal.targetBinding)) 'R6_CURRENT_CANONICAL_V3_THREE_STAGE_RESERVATION_FAILURE_CONTRACT_FAILED'
    }
    $text = ($output | ForEach-Object { $_.ToString() }) -join "`n"
