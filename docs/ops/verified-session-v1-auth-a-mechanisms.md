...
| Supabase control plane | `GET /v1/projects`; then `GET /v1/organizations/{organization_slug}` derived from the unique target project | Two requests, no pagination, redirects or retry | Existing read-capable `SUPABASE_ACCESS_TOKEN`; value never printed | `scripts/qa/verified-session-auth-a-supabase-read.mjs` with loopback fixtures. Proves inventory only; capacity remains `UNKNOWN`. Production mode disabled. |
...
- Production: hard-disabled in each new client. `scripts/qa/verified-session-auth-a-execute.mjs` is a non-executed LOCAL_TEST contract: it checks a new ID (never `auth-a-verified-session-001`), strict UTC, exact source/packet identity and `AUTH_A_EXECUTE=1`, orders the local mock steps, and fails closed without a reviewed Worker identity mapping. It rejects Production before any read. It does not yet wire real catalog-row normalization or migration-history provenance and is not a runnable hosted AUTH-A mechanism. Future Production clients must use constant HTTPS origins only; caller-injected origins are rejected.
