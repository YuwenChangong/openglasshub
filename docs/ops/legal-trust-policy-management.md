...
The GET re-audit clears the active source blocker under remediation commit `aa2cf01be3a21eed9d3a0de111bc249c0ead046a`; legal consent remains unrelated to public resource authorization. All three forward comment migrations remain authored, committed, and unexecuted: `20260713_comment_creation_circle_authorization.sql` protects POST creation, `20260713_comment_reaction_visibility_authorization.sql` protects PUT reaction access, and `20260713_comment_read_circle_visibility_authorization.sql` protects GET public read ancestry. They remain production deployment prerequisites. Batches 1-4 and the Batch 5 `comments.ts` source are complete at 46/66 traced and 20 pending. The next pending Batch 5 source is `src/pages/api/forum/external-video-upload.ts#POST`; Batch 5 remains pending, Batch 6 remains pending, and production remains NO_GO.
...
## Phase 4A1 external video authorization ordering blocker

`src/pages/api/forum/external-video-upload.ts#POST` remains blocked pending re-audit, but its runtime ordering remediation is implemented. It now authenticates the bearer, checks global write safety, validates the payload, looks up the selected post, and proves server-read `post.author_id === authData.user.id` before it conditionally calls `validateTurnstileToken`. That helper makes a fixed-host Cloudflare Turnstile `fetch`, but target-resource authorization now completes before it. This is not SSRF because the endpoint accepts no external URL and the helper destination is hard-coded.

The fully offline ordering regression test proves missing, non-owner, malformed, unauthenticated, and safety-denied requests perform zero later Turnstile, Cloudflare-fetch, daily-rate, R2-signing, rate-attempt, or direct mutation calls. A valid owner reaches Turnstile only after lookup and ownership proof; invalid Turnstile stops before the rate-attempt insert; and a Turnstile-disabled path still authorizes the target before later processing. No migration change is required for this route-local blocker. A separate POST re-audit remains required, so the blocker stays active; the later rate-attempt insert remains the first persistent effect, R2 signing does not upload an object, and the endpoint does not insert `post_media`. Phase 4A1 remains at 46/66 traced and 20 pending, Batch 5 is pending, and production remains NO_GO.
