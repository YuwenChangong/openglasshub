...
`src/pages/api/forum/comments.ts#POST` is complete after re-audit. It derives the actor only from verified bearer authentication; safety, payload, profile, and server-derived post-circle-parent checks all complete before moderation, the rate-attempt write, and `comments.insert`. The resolver denies missing, mismatched, deleted, hidden, unpublished, inactive, or canonical-hidden ancestors, and an optional parent must be published and tied to the same post. The insert uses only verified `author_id` plus resolver-returned target ids. `20260713_comment_creation_circle_authorization.sql` provides matching INSERT RLS for authenticated actor ownership, valid comment state, published post, active canonical-visible circle, and same-post published reply parent. The schema has no private-circle membership relation. The migration is authored but unexecuted; applying it remains a deployment prerequisite, not an active source blocker. GET is the final incomplete Batch 4 method, progress is 44/66 traced and 22 pending, and production remains NO_GO.

## Phase 4A1 public comment-read ancestor visibility blocker

`src/pages/api/forum/comments.ts#GET` remains incomplete and blocks Batch 4 closure. Its public/anonymous path validates `post_id` and checks a published, moderation-published post, but the post read omits `circle_id` and the route never validates the parent circle through `isPublicVisibleCircle` or an equivalent canonical visibility helper. The public post and comment SELECT policies likewise lack post-to-circle and comment-to-post-to-circle predicates. A runtime repair and one new forward SELECT-RLS migration are both required; changing only one layer is insufficient, historical migrations must not be edited, and no migration execution is authorized in this checkpoint.

Both existing forward comment migrations remain authored, committed, and unexecuted: `20260713_comment_creation_circle_authorization.sql` protects POST creation and `20260713_comment_reaction_visibility_authorization.sql` protects PUT reaction access. They do not repair GET's post/comment SELECT chain. GET requires a separate source re-audit after runtime and forward-policy remediation; until then Batch 4 remains pending at 44/66 traced and 22 pending, Batches 1-3 remain complete, Batches 5-6 remain pending, and production remains NO_GO.
