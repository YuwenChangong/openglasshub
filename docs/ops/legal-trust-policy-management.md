...
The migration inventory is now twelve authored, unexecuted prerequisites. The twelfth migration statically hardens three `SECURITY DEFINER` execute ACLs after the moderation notification writer became a verified-actor, server-only service-role boundary. It does not validate a live database, replace legal/operator contact data, or remove the requirement for qualified legal review.
...
## Phase 4B Wave 2 Rollout

Wave 2 integrates the next six manifest methods, one per source file: `src/pages/api/admin/reports/[id]/action.ts#POST`; `src/pages/api/admin/users/[id]/ban.ts#POST`; `clear-warning.ts#POST`; `suspend.ts#POST`; `unban.ts#POST`; and `warn.ts#POST`. Each preserves `requireModerator` as the existing bearer-derived staff authorization boundary and immediately follows it with the centralized current-consent guard, using the verified `auth.user.id` and bearer-bound RLS client only. JSON parsing, report/target lookup, safety role/state reads, transition work, report events, safety events, notifications, audits, and persistent mutations all remain after a successful current-consent result.

The report action keeps report id route-derived and action enum server-allowlisted; current consent cannot replace moderator authority, report status authority, target ownership, action transition checks, or target moderation/user-safety authorization. The user-action routes keep route-derived target ids and helper-enforced server re-reads of actor and target roles. The safety hierarchy remains moderator to ordinary user only where allowed, admin to ordinary user or moderator, self-target denied, admin target protected, and missing/unknown role denied before safety-state access. The Wave 2 offline harness proves no-auth 401, insufficient-staff denial before consent, current continuation, missing/stale 403, sanitized reader-failure 503, request override resistance, action-specific downstream boundaries, and zero downstream effects on consent denial.

With Wave 2, Phase 4A1 remains 66/66 complete, Phase 4A2 remains 5/5 complete, Wave 1 remains 10/10, Wave 2 is 6/6, cumulative guard integration is 21/37, and 16 mutations remain. Consent POST, auth/recovery/resend operations, and read-only GETs remain exempt. No migration was added or executed; authored migrations remain deployment prerequisites. Wave 3 begins at `src/pages/api/forum/circles/[slug]/comments.ts#PATCH`, Phase 4B is incomplete, no real service operation occurred, and production remains `LEGAL_TRUST_CONSENT_FOUNDATION_V1_PHASE4B_NO_GO`.

## Phase 4B Wave 3 Rollout

Wave 3 integrates the PATCH/DELETE pairs for managed circle comments, managed-circle administration, and managed circle posts, plus general forum circle POST/PATCH. Each slug mutation now separates bearer verification from circle lookup: `requireForumUser`, then the centralized verified-actor consent guard, then `requireManagedCircleForAuthenticatedUser`. The original wrapper still composes those steps for GET, so no read-only route is accidentally guarded.

General circle POST/PATCH await the same guard after `requireForumUser` and before user-safety checks. Missing, stale, or unavailable consent therefore cannot reach any circle/post/comment target lookup after insertion, managed-circle authority, safety read, duplicate/rate work, moderation/provider request, storage cleanup, persistent mutation, notification, audit, or other downstream effect. Current consent cannot replace exact slug/circle binding, cross-circle rejection, author/staff or owner/staff authority, lifecycle rules, server-assigned owner fields, or user-safety denial.

The shared offline matrix and forum-permission audit prove exact 401/403/503 behavior, zero-effect denials, verified actor/bundle scope, current-consent continuation, and GET/auth/bootstrap exemptions. Totals are Phase 4A1 66/66, Phase 4A2 5/5, Wave 1 10, Wave 2 6, Wave 3 8, cumulative 29/37, with eight remaining. Wave 4 is `src/pages/api/forum/comments.ts#DELETE`, `#PUT`; `src/pages/api/forum/external-video-upload.ts#POST`; `media-upload-guard.ts#POST`; `post-media.ts#POST`; `src/pages/api/forum/posts.ts#DELETE`, `#PATCH`; and `src/pages/api/users/me/notifications.ts#PATCH`. No migration was added or executed; the authored migration inventory remains a deployment prerequisite, no real service operation occurred, and production remains `LEGAL_TRUST_CONSENT_FOUNDATION_V1_PHASE4B_NO_GO`.

## Phase 4B Wave 4 Completion

The final eight mutations now use the existing centralized required-consent guard after verified authentication and before any route-specific safety, resource, storage, provider, rate, persistence, cleanup, or notification effect: comment DELETE/PUT, external-video upload POST, media-upload guard POST, post-media POST, post DELETE/PATCH, and self-notification PATCH. Only the verified server actor and bearer-bound RLS client enter the guard. Existing authentication remains 401; missing/outdated consent returns exact `403 LEGAL_CONSENT_REQUIRED` plus `/legal-consent/`; unavailable consent returns sanitized `503 LEGAL_CONSENT_UNAVAILABLE`.

Current consent never replaces ownership, staff/moderator, lifecycle, circle visibility, post-bound media provenance, upload-purpose, Turnstile, rate, or recipient-isolation rules. Offline call-log matrices verify no consent reader before 401 and no downstream work on 403/503. Endpoint fakes specifically prove no external-video Turnstile/fetch/signing/rate effect and no notification recipient update when consent is denied. Bootstrap consent POST, auth/recovery methods, and read-only GETs stay unguarded.

The deterministic inventory is closed: Phase 4A1 66/66 traced, Phase 4A2 5/5, Wave 1 10/10, Wave 2 6/6, Wave 3 8/8, Wave 4 8/8, and Phase 4B 37/37 with zero remaining source integrations. The profile/service-role audit remains passing. No real service, migration, deployment, or production operation occurred, and the next phase was not started.

Deployment prerequisites remain authored but unexecuted: `20260703_moderation_action_notifications.sql`, `20260712_legal_policy_acceptances.sql`, `20260713_comment_creation_circle_authorization.sql`, `20260713_comment_reaction_visibility_authorization.sql`, `20260713_comment_read_circle_visibility_authorization.sql`, `20260713_forum_posts_circle_authorization.sql`, `20260713_forum_report_target_authorization.sql`, `20260713_post_bound_media_provenance.sql`, `20260714_circle_cover_public_visibility_authorization.sql`, `20260715_post_media_delivery_visibility_authorization.sql`, and `20260716_profile_media_delivery_authorization.sql`. Source rollout completion is not production readiness; configuration, legal review, migration execution, consent persistence, and production verification remain outstanding. Classification remains `LEGAL_TRUST_CONSENT_FOUNDATION_V1_PHASE4B_NO_GO`.
