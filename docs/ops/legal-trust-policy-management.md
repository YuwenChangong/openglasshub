...
`src/pages/api/forum/external-video-upload.ts#POST` is blocked. It authenticates the bearer and checks global write safety, but conditionally calls `validateTurnstileToken` before it loads the client-selected post and proves server-read `post.author_id === authData.user.id`. That helper makes a fixed-host Cloudflare Turnstile `fetch`, so an authenticated but unauthorized caller can cause an outbound effect before target-resource authorization. This is not SSRF because the endpoint accepts no external URL and the helper destination is hard-coded; it is an authorization-ordering violation.

Runtime reordering is required. Target post existence and ownership must be proven before Turnstile. Denied targets must cause zero Turnstile, video, R2, rate-attempt, database, or later effects. No migration change is required for this specific blocker, and a separate POST re-audit is required after the reorder. The later rate-attempt insert is the first persistent effect; R2 signing does not upload an object and the endpoint does not insert `post_media`. Phase 4A1 remains at 46/66 traced and 20 pending, Batch 5 is pending, and production remains NO_GO.
