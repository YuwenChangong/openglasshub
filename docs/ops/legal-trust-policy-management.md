...
## Phase 4A1 comment-creation ancestor authorization blocker

`src/pages/api/forum/comments.ts#POST` remains incomplete and blocks Phase 4A1 Batch 4 pending a separate re-audit. Runtime remediation is implemented: after verified bearer authentication, actor safety, payload validation, and profile lookup, `resolveAccessibleCommentCreationTarget` server-derives the post, circle, and optional parent before moderation, rate persistence, or insertion. It requires a published post, active canonical-visible circle, and a published same-post parent. The insert derives `author_id` from verified auth and target ids from that resolver. The forward migration `20260713_comment_creation_circle_authorization.sql` is authored but unexecuted; it replaces comments INSERT RLS with the same actor, post, circle, and parent predicate. The current schema has no private-circle membership relation, so no private-member path is supported. Historical migrations remain untouched. Migration execution and a separate POST re-audit are still required; GET remains incomplete, and production remains NO_GO.
