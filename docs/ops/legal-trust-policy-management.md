...
`src/pages/api/forum/comments.ts#POST` remains incomplete and blocks Phase 4A1 Batch 4. It authenticates and safety-checks the actor and rejects non-published or moderation-hidden posts, but it never binds the target post to an active accessible circle. `comments_insert_self` in `20260616_community_moderation_mvp.sql` likewise enforces only `author_id = auth.uid()` and comment status, so a published post retained under a deleted circle can receive a top-level comment or reply. Runtime and forward-RLS remediation are both required; historical migrations must not be edited, a new forward migration will be required, and no migration execution is authorized in this checkpoint. A separate POST re-audit is required after remediation, and GET must still be traced before Batch 4 can close. Production remains NO_GO.
