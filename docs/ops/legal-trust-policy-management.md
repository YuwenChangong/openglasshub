...
## Phase 4A1 comment reaction visibility blocker

`src/pages/api/forum/comments.ts#PUT` is an active release-blocking authorization mismatch. The verified bearer actor is correct, but PUT authorizes a reaction from the comment's publication fields alone. It does not prove that the comment's parent post remains published, moderated-published, visible, or accessible. `GET` in the same route does enforce that parent-post visibility contract. A hidden or soft-deleted post may retain published comments, allowing PUT to write a reaction for content that GET treats as inaccessible.

The forward RLS policy `comment_reactions_insert_self` in `supabase/migrations/20260603_forum_comments_interactions.sql` repeats the gap: it requires `user_id = auth.uid()` and a published comment, but does not join the parent post or enforce post/circle visibility. Remediation must therefore include both a runtime target-post visibility check and a forward RLS migration that enforces the matching predicate. Fixing only one layer is insufficient, and legal-consent enforcement is not a substitute for resource authorization. The evidence checkpoint preserves 42/66 traced and 24 pending, keeps `PUT` incomplete, and leaves Batch 4 pending. No remediation is included in this checkpoint.
