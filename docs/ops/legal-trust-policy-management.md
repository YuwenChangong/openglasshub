...
Runtime and forward-RLS remediation are now implemented, but a separate re-audit is still required. `resolveAccessibleCommentReactionTarget` resolves the comment, parent post, and circle from server reads before any reaction query; `supabase/migrations/20260713_comment_reaction_visibility_authorization.sql` installs a matching full-chain predicate for reaction SELECT, INSERT, UPDATE, and DELETE, while mutation policies require `user_id = auth.uid()`. Fixing only one layer is insufficient, and legal-consent enforcement is not a substitute for resource authorization. The evidence checkpoint preserves 42/66 traced and 24 pending, keeps `PUT` incomplete, and leaves Batch 4 pending.
