...
The GET re-audit clears the active source blocker under remediation commit `aa2cf01be3a21eed9d3a0de111bc249c0ead046a`; legal consent remains unrelated to public resource authorization. All three forward comment migrations remain authored, committed, and unexecuted: `20260713_comment_creation_circle_authorization.sql` protects POST creation, `20260713_comment_reaction_visibility_authorization.sql` protects PUT reaction access, and `20260713_comment_read_circle_visibility_authorization.sql` protects GET public read ancestry. They remain production deployment prerequisites. Batches 1-4 and the Batch 5 `comments.ts`, external-video, and media-upload-guard sources are complete at 48/66 traced and 18 pending. The next pending Batch 5 source is `src/pages/api/forum/post-media.ts#POST`; Batch 5 remains pending, Batch 6 remains pending, and production remains NO_GO.
...
## Phase 4A1 post-media cross-post provenance blocker

`src/pages/api/forum/post-media.ts#POST` remains incomplete and blocked. The external-video issuer authorizes a verified actor for a selected owned post but signs an object whose key is only `tmp/<actor-id>/<uuid>-<normalized-file-name>`. The post-media consumer accepts any same-actor temporary key for any selected post the actor owns. Its target authorization proves ownership of the destination post, not that the stored object was authorized for that destination.

The active `post_media_insert_self` policy requires authenticated row ownership and an authenticated author-owned destination post, and `post_media_objects_insert_self` requires authenticated storage ownership plus a user-id path prefix. Neither policy records or checks the original post that authorized a temporary object. A post A upload can therefore be replayed onto owned post B. No current upload session, post-bound key, nonce, or object-to-post provenance record closes this gap. Runtime finalization/provenance enforcement and a forward RLS/schema predicate are both required; consent cannot replace resource authorization. Counts remain 48/66 traced and 18 pending, Batch 5 remains pending, and the three authored comment migrations remain unexecuted deployment prerequisites.
