...
Runtime remediation is now implemented in `resolveAccessibleCommentReactionTarget`: after verified bearer authentication and UUID validation, `PUT` derives the published comment, published parent post, and active visible circle from server reads before every `comment_reactions` query or mutation. The forward migration `20260713_comment_reaction_visibility_authorization.sql` supersedes the supported reaction SELECT, INSERT, UPDATE, and DELETE policies with the same full-chain predicate, and mutation policies retain `user_id = auth.uid()`. This remains an active release blocker until a separate source-and-policy re-audit is completed; implementation does not complete the legal-consent trace. Legal-consent enforcement cannot establish resource accessibility. `PUT` remains incomplete and no other method is marked complete. Progress remains 42/66 traced and 24 pending; Batches 1-3 are complete and Batch 4 remains pending.
