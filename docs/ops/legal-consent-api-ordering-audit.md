...
## Active release blocker - forum/comments.ts#PUT reaction visibility

`PUT` authenticates the bearer, derives `authData.user.id`, validates `comment_id`, and reads only `comments.id`, `status`, and `moderation_status`. A published comment reaches `comment_reactions.insert({ comment_id, user_id: authData.user.id, reaction_type: "like" })` without a parent-post or circle lookup. That insert is the first reaction mutation. In contrast, the same source's `GET` resolves the parent post and returns 404 unless both post status and moderation status are published, apart from the verified author's pending-review post. A hidden or soft-deleted post can retain its published comments because the circle post delete route changes only `posts.status`.

The database does not restore the missing authorization. `comment_reactions_insert_self` in `20260603_forum_comments_interactions.sql` requires `user_id = auth.uid()` and an existing published comment, but contains no parent-post status, post moderation, or circle visibility predicate. Therefore route-only remediation leaves direct RLS-backed API access open, while RLS-only remediation leaves the route's resource contract incomplete. This is an active release blocker: runtime remediation must bind the target comment to a currently visible parent post before mutation, and a forward RLS migration must enforce the same predicate for reaction INSERT. Legal-consent enforcement cannot establish resource accessibility and does not remediate this authorization mismatch. `PUT` remains incomplete and no other method is marked complete. Progress remains 42/66 traced and 24 pending; Batches 1-3 are complete and Batch 4 remains pending.
