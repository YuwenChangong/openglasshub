...
- Signup: terms/community acceptance without age attestation, a six-digit Supabase signup confirmation **code**, and session verification only after the provider accepts that code with the server-selected `type:"signup"`. Signup confirmation is not the fresh-login challenge.
...
Allowed transitions: anonymous -> pending after password; pending -> verified through the atomic fresh-login challenge transaction, or through the separate initial-signup path after Supabase accepts the genuine signup artifact with server-hardcoded `type:"signup"`, policy acceptance is recorded, and only the returned signup session is marked verified; verified -> verified on refresh of the same session; verified/pending -> anonymous on logout/expiry; any other new session -> pending. A verified record can never migrate to a different session ID. Generic email OTP, magic link, recovery, email change, an arbitrary OTP-authenticated session, or another identity's code is not signup proof. The returned `amr=otp` describes the authentication method, not signup provenance.
...
The fresh-login code is sent from the Worker through **Brevo's free transactional HTTPS API**, using a new server-only API key and the already verified sender. Existing Supabase Auth -> Brevo SMTP remains unchanged for signup confirmation/reset; the reviewed signup-confirmation template must expose six-digit `{{ .Token }}`. These two email mechanisms and their resend paths remain separate. Do not switch to Supabase's default mailer, add another vendor, or log the code/password/API key. Brevo's [transactional endpoint](https://developers.brevo.com/reference/send-transac-email) is available with a [300-email/day Free quota](https://help.brevo.com/hc/en-us/articles/208580669-FAQs-What-are-the-limits-of-the-Free-plan); the design caps custom challenge sends at 100/day to reserve headroom for existing Auth mail. This cap is not a guarantee that shared SMTP volume stays below 300: provider quota/queue uncertainty fails closed, and expired delayed codes are rejected. No automatic paid fallback or other security/login-success email is added.

## 7. User journeys

| Journey | Required sequence |
| --- | --- |
| Signup | Collect email/password and Terms/Community Guidelines acceptance without age declaration. Supabase `signUp` sends a six-digit `{{ .Token }}` signup code via the existing Brevo SMTP path. The user enters the code; the server endpoint accepts only email and code from the client, calls `supabase.auth.verifyOtp({email,token:code,type:"signup"})` with the type hardcoded server-side, and treats provider acceptance as the signup-provenance boundary. It records non-age policy acceptance as applicable, marks only the returned signup session verified, then follows a sanitized internal redirect. Direct provider verification outside this endpoint still yields a pending session, not access. Neither the client nor a browser flag, metadata, raw JWT decode, `amr=otp`, or a successful `type:"email"` verification may select or substitute the provenance decision. |
| Signup resend | Use provider-native `supabase.auth.resend({type:"signup",email})`, then verify the resent six-digit code with server-hardcoded `type:"signup"`. A superseded code must not activate signup. Provider resend limits and the existing app-level abuse control both apply; this is separate from fresh-login challenge resend. |
...
- `PROVIDER_NATIVE_SIGNUP_PROVENANCE=true`
- `SIGNUP_VERIFICATION=Supabase six-digit signup code; server-hardcoded type:"signup"; returned-session-bound activation`
...
| 21. Signup flow confused with generic OTP | `type:"email"` accepts genuine signup codes but does not establish their purpose; `amr=otp` is also insufficient | Genuine signup artifact plus server-hardcoded provider `type:"signup"`; no client-selected type or app-owned intent | Genuine six-digit code succeeds; generic OTP, magic link, recovery, email change, cross-email, replay, and superseded codes are rejected as signup. |
...
Before Production approval, prove locally: grants, policy coverage, function ownership, live auth-session lookup, trusted claim extraction, the provider-native signup contract and template, and free-tier behavior. The signup-provenance architecture decision is resolved for the reviewed local stack, not for an unverified hosted configuration. In local/preview, exercise every authorization matrix row with anonymous, pending, verified, staff, and direct REST/Storage actors; preserve public reads and bound Brevo sends to test mailboxes. The Production migration, RLS/RPC/Storage grants, API-key binding, signup template, hosted provider compatibility, and exact order require separate review and authorization. The implementation plan must derive the cutover mechanism from the repository rather than assume a particular feature gate.
...
- Provider-native signup provenance was proven in a disposable local stack: Supabase CLI 2.115.0, Supabase Auth v2.195.0, `@supabase/supabase-js` 2.112.4, Node v24.14.1. Actual six-digit `{{ .Token }}` signup codes verified with `type:"signup"`, confirmed the same user, returned a session, and rejected replay; resent codes worked while prior codes failed. Valid generic OTP, magic-link hash, recovery, email-change, and cross-email artifacts were rejected as signup. The returned `amr=otp` was not used as provenance, and `type:"email"` also accepted a genuine signup code, so it is **not** the signup boundary. This is local compatibility evidence, not Production configuration verification. See [email templates](https://supabase.com/docs/guides/auth/auth-email-templates) and [verifyOtp](https://supabase.com/docs/reference/javascript/auth-verifyotp).
- Implementation and pre-Production verification must confirm the actual deployed Supabase/client configuration still accepts genuine signup artifacts with server-hardcoded `type:"signup"` and rejects non-signup artifacts before release activation. If a dependency upgrade or hosted behavior breaks that contract, fail closed and return to architecture review; never fall back to `type:"email"`, generic OTP claims, a client-selected type, or a new signup-intent/Send Email Hook subsystem without separate design approval.
