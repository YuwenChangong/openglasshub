...
On a disposable Supabase CLI 2.115.0/Auth 2.195.0 instance, with locked JS/Auth 2.112.4 and Node 24.14.1, `auth.getClaims(token)` accepted a genuine password JWT and rejected signature tampering, forged `session_id`, forged `amr`, and an expired signed token. Authorization uses verified standard claims only, not editable `user_metadata`. The local symmetric signing path has `getClaims` verify through Auth; a future asymmetric hosted key uses the official JWKS path. Neither a raw decode nor `getUser` plus separately decoded payload is authorized. Select **Class A only**. The earlier local proof establishes cryptographic verification and session behavior; Task 1 additionally tests the full OpenGlass claim-shape contract below.
...
- Header/private UI: `src/components/site/HeaderUserMenu.tsx`, `HeaderNotifications.tsx`, `SiteHeader.astro`, `src/components/notifications/NotificationsPage.tsx`, `src/components/forum/PostSocialActions.tsx`, `CommentsSection.tsx`, `src/components/profile/MyProfilePage.tsx`, `EditProfileForm.tsx`, `src/layouts/CommunityLayout.astro`, `src/components/legal/LegalConsentGate.tsx`, `LegalConsentPage.tsx`.
...
| 6, 12, 13 | `HeaderNotifications.tsx`, `NotificationsPage.tsx`, `PostSocialActions.tsx`, `CommentsSection.tsx`; publication on `forum_notifications`, `comments`, `post_votes`, `comment_reactions` | Task 6 enforces Postgres-Changes visibility in RLS; Task 12 suppresses client subscriptions until verified and tears them down after logout/state loss; Task 13 verifies direct Realtime bypass denial. Public interaction visibility remains unchanged. |
...
All `SECURITY DEFINER` functions are owned by `postgres`, `SET search_path = ''`, schema-qualify every object, and start with `REVOKE ALL ... FROM PUBLIC, anon, authenticated`. Grant only the listed role. Service-role functions accept `p_user_id/p_session_id` only after Worker Class A verification, and re-check `auth.sessions`; user-token predicate accepts no identity parameters. SQL errors map to stable non-enumerating API codes. Every mutation function captures `v_now := clock_timestamp()` once at entry and explicitly uses that one database-owned instant for creation, expiry/cooldown comparisons, attempts, UTC day via `(v_now AT TIME ZONE 'UTC')::date`, verification, revocation, and acceptance. Table timestamp defaults are defensive only; security transitions explicitly write `v_now`. The caller cannot supply a security clock.
...
| `public.ogh_reserve_login_challenge(p_user_id uuid,p_session_id uuid,p_challenge_id uuid,p_digest bytea,p_ip_hash text,p_resend boolean) RETURNS text` | DEFINER, service_role only; Worker challenge service | Worker generates a random challenge UUID before computing HMAC over that ID, user, session, and code; verify live owned `auth.sessions`; serialize on session advisory transaction lock then challenge row; enforce password precondition at Worker; start idempotent, resend >=60s, <=3 sends/session, <=5/user/day, <=10/IP-hash/day, <=100/global/day; derive UTC quota day from `v_now`; reserve all budget counters and a new unusable-until-accepted digest in one transaction. Deterministic `RESERVED|PENDING|RESEND_COOLDOWN|EMAIL_BUDGET_EXHAUSTED|SESSION_GONE`. No send on non-RESERVED. |
| `public.ogh_finalize_login_delivery(p_user_id uuid,p_session_id uuid,p_challenge_id uuid,p_accepted boolean) RETURNS boolean` | DEFINER, service_role only; Worker after one provider attempt | Match same challenge under lock; accepted marks usable, rejected/ambiguous marks unusable; never credit back uncertain quota or auto resend. Old challenge superseded atomically on successful reservation. |
| `public.ogh_consume_login_challenge(p_user_id uuid,p_session_id uuid,p_challenge_id uuid,p_digest bytea) RETURNS text` | DEFINER, service_role only; Worker verification route | Lock session then challenge; verify live ownership, accepted delivery, not superseded/consumed, `v_now<expires_at`, digest equality via server-computed HMAC, and attempt count; wrong attempt increments exactly once, final attempt exhausts; correct code consumes and inserts one verified row with DB-owned `verified_at` in the same transaction. `VERIFIED|CHALLENGE_INVALID|CHALLENGE_EXPIRED|CHALLENGE_EXHAUSTED|CHALLENGE_SUPERSEDED|SESSION_GONE`. Logout/verify race is serialized by session lock. |
| `public.ogh_activate_signup_session(p_user_id uuid,p_session_id uuid) RETURNS boolean` | DEFINER, service_role only; server endpoint after provider `type:"signup"` result | Require exact returned user/session and live `auth.sessions` ownership, insert `verification_kind='signup'` with DB-owned `verified_at` once. No browser EXECUTE or arbitrary OTP session activation. |
| `public.ogh_revoke_verified_session(p_user_id uuid,p_session_id uuid) RETURNS boolean` | DEFINER, service_role only; logout endpoint | Lock session, require owned row, set DB-owned `revoked_at` before `auth.signOut({scope:'local'})`; idempotent only for the same actor/session; other sessions unaffected. |
| `public.ogh_record_policy_acceptance(p_user_id uuid,p_bundle text,p_terms text,p_privacy text,p_guidelines text,p_source text) RETURNS void` | DEFINER, service_role only; consent bootstrap route | Worker validates signed identity and explicit checkbox; upsert only this user/bundle with DB-owned `accepted_at`, current versions and allowed source; historical reader remains read-only. No age input. |
...
**Interfaces:** Consumes `admin-auth.ts` `RuntimeEnv`, `requireEnv`, `getBearerToken`, `createUserClient`; produces `VerifiedSessionClaims {userId:string;sessionId:string;amr:readonly {method:string;timestamp?:number}[];expiresAt:number}`, `getTrustedSessionClaims(token,env)`, `getCurrentConfirmedAuthUser(token,env,claims): Promise<{id:string;email:string}>`, `requireVerifiedSession(request,env)` and `SessionState`. `getTrustedSessionClaims` uses `auth.getClaims(token)` for cryptographic verification. It derives expected `iss` as `${new URL(requireEnv(env, "SUPABASE_URL")).origin}/auth/v1` from trusted server configuration; requires exact issuer, `aud === 'authenticated'` or a supported string array containing `'authenticated'`, `role === 'authenticated'`, UUID `sub` and `session_id`, numeric unexpired `exp`, numeric `nbf` not in the future when present, `is_anonymous === false`, and an array of provider AMR objects with a nonempty string `method` (and valid numeric timestamp when present). Reject missing/malformed fields; password `amr` is additionally required at challenge start. Do not construct issuer from request data. `getCurrentConfirmedAuthUser` separately calls `auth.getUser(token)` **only** for current provider account data; require `providerUser.id === claims.userId`, nonempty provider `email`, and a valid nonempty `email_confirmed_at` from the installed Supabase User contract (not phone-only `confirmed_at`). It fails closed on absent/inconsistent fields and returns only the provider ID/email. Neither this helper nor `getUser` supplies `session_id` or `amr`. `requireVerifiedSession` calls user-token `public.ogh_is_verified_session()`; never trusts browser state.

- [ ] RED: add signed valid/tampered/forged-session/forged-amr tests and assert `WRONG_ISSUER=REJECTED`, `WRONG_AUDIENCE=REJECTED`, `WRONG_ROLE=REJECTED`, `MISSING_SESSION_ID=REJECTED`, `MALFORMED_SUB=REJECTED`, `MALFORMED_SESSION_ID=REJECTED`, `ANONYMOUS_TOKEN=REJECTED`, `FUTURE_NBF=REJECTED`, `EXPIRED_TOKEN=REJECTED`, and `USER_METADATA_AUTHZ_EFFECT=NONE`; include DB-unavailable/pending/stale tests. Test confirmed current provider email succeeds; request-body/JWT email cannot select the delivery address; `getUser` failure, ID mismatch, unconfirmed email, and missing email yield no deliverable address. `node --experimental-strip-types scripts/test-verified-session-claims.mjs` must fail because exports are absent.
- [ ] Implement additional OpenGlass claim-shape validation after official `getClaims` verification and the separate current-user lookup. Never use `getUser` for session/AMR, raw JWT decode, `getSession().user`, `user_metadata`, or caller-supplied identity fields as authorization sources.
- [ ] GREEN: run `node --experimental-strip-types scripts/test-verified-session-claims.mjs`; all valid/negative controls pass on mocked unit cases and disposable signed local tokens, including `USER_METADATA_AUTHZ_EFFECT=NONE`.
...
- [ ] RED: local concurrent tests for two starts, simultaneous resend, same-code double consume, superseded code, cross-session code, final-attempt race, budget race, logout-vs-verify, session disappearance between start/consume, and policy-only historical/new acceptance. Use disposable DB fixture timestamps/transaction-local test preparation for expiry and UTC-day boundaries; no Production RPC clock parameter. Assert Worker cannot choose expiry, bypass cooldown, select another quota day, or backdate/future-date verified/revoked/accepted timestamps; DB clock governs all transitions. Assert `AT_MOST_ONE_VERIFICATION_TRANSITION_PER_CHALLENGE_SESSION=true` and no browser service-only EXECUTE. `node scripts/test-verified-session-sql.mjs` fails with missing functions.
- [ ] Implement one `clock_timestamp()` capture per function invocation, transaction advisory lock keyed by session, row locking, atomic daily upsert counters using DB-derived UTC day, delivery-state transition, and one-transaction consume/verified insert. Avoid a function per UI action beyond the six listed.
...
**Interfaces:** Consumes Task 1 `VerifiedSessionClaims` and `getCurrentConfirmedAuthUser(token,env,claims)`, Task 3 service-only public RPCs without caller time; produces `sendFreshLoginCode({to,code,requestId},env,fetchImpl)`, `startChallenge`, `resendChallenge`, `verifyChallenge` with typed API errors. Start/resend use `getClaims` identity and password AMR plus live pending session first, then `getUser` only for the current confirmed provider email; require provider ID to match signed `sub`. `BREVO_API_KEY`, `BREVO_VERIFIED_SENDER_EMAIL`, and `OGH_LOGIN_CODE_PEPPER` are server-only bindings.

- [ ] RED: mock fetch/DB for missing secret, timeout, 2xx acceptance, 4xx rejection, ambiguous network/5xx response, quota/cooldown, unknown account parity, absent password `amr`, `getUser` failure, provider ID mismatch, absent/unconfirmed provider email, JWT email different from provider email, and hostile request-body email. Assert no Brevo call on every identity/email failure and exactly the current confirmed provider email as recipient on success. Run `node --experimental-strip-types scripts/test-verified-session-brevo.mjs`; missing exports fail.
...
**Interfaces:** POST start/resend/verify consume bearer token. Start/resend use Task 1 `getTrustedSessionClaims` for signed identity/session/password AMR, check an owned live pending session, then call `getCurrentConfirmedAuthUser(token,env,claims)` for the **current provider user record** and require its confirmed `email` and matching `id`. `getUser` never supplies `session_id` or `amr`; any JWT email is ignored for delivery. Start/resend request bodies cannot set authoritative email/user/session/amr/confirmed state; reject such fields rather than silently using them. Verify accepts only opaque challenge ID and six-digit code. No endpoint grants normal privileged access except atomic verified transition.

- [ ] RED: test missing/forged claims, request-body email/user/session/amr/confirmed fields rejected, JWT email mismatch ignored, `getUser` failure, provider ID mismatch, missing/unconfirmed provider email, exact current confirmed provider email recipient, no password method, stale session, duplicate start, cooldown, wrong/expired/replayed/cross-session code, provider ambiguity, quota, and DB failure. Every invalid identity/email case has zero Brevo calls; no account-enumerating response. Focused route suite fails on missing endpoints.
...
**Files:** Modify `src/components/auth/useBrowserAuthState.ts`, `src/components/site/{HeaderUserMenu,HeaderNotifications,SiteHeader.astro}`, `src/components/notifications/NotificationsPage.tsx`, `src/components/forum/{PostSocialActions,CommentsSection}.tsx`, `src/components/profile/{MyProfilePage,EditProfileForm}.tsx`, `src/layouts/CommunityLayout.astro`; extend `scripts/test-verified-session-ui.mjs`, existing `scripts/test-mobile-header-auth.mjs`.
...
- [ ] RED: test anonymous/pending/verified/stale/DB-unavailable, profile-summary 500, no zero-stat fabrication, notifications hidden pending, profile/admin navigation absent pending, and no `PostSocialActions`/`CommentsSection` Realtime subscription until verified (with teardown on logout or state loss). Run `node scripts/test-mobile-header-auth.mjs` and focused UI suite; expect old `getSession`-alone behavior to fail.
- [ ] Implement state-driven header and private-page gates; retain OpenGlass navigation, icons, and layout. Suppress private Realtime subscriptions until verified and tear them down on logout or verified-state loss.
...
- [ ] RED: add every spec threat-model case not already covered, especially email-budget race, same code twice concurrently, old signed JWT after signout, stale session, owner/role preservation, direct RPC/Storage/private-notification Realtime, user metadata, summary failure, signup negative artifacts. Run all four suites; any uncovered bypass fails.
