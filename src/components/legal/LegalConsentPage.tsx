...
import { getLegalConsentStatus, LegalConsentClientError, recordLegalConsent } from "../../lib/legal-consent-client";
...
export default function LegalConsentPage({ next, reason, authAdapter, consentAdapter, navigationAdapter }: { next?: string; reason?: string; authAdapter?: LegalConsentAuthAdapter; consentAdapter?: LegalConsentAdapter; navigationAdapter?: LegalConsentNavigationAdapter }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setError("");
    try {
      if (consentAdapter) await consentAdapter.recordCurrentConsent({ accessToken: token, source: sourceForReason(reason ?? null) });
      else await recordLegalConsent({ accessToken: token, source: sourceForReason(reason ?? null) });
      setState("current");
  ...
  return <section className="auth-card"><div className="auth-card__top"><h1>政策确认</h1><p>请阅读并确认当前政策版本。</p></div><form className="auth-form" onSubmit={submit}><div className="auth-legal-acknowledgement"><input id="legal-consent-acknowledgement" type="checkbox" checked={acknowledged} onChange={(event) => { setAcknowledged(event.target.checked); if (event.target.checked) setError(""); }} aria-invalid={error ? true : undefined} aria-describedby={error ? "legal-consent-error" : undefined} /><label htmlFor="legal-consent-acknowledgement">我已阅读并同意 <a href={LEGAL_POLICY.routes.terms} target="_blank" rel="noopener noreferrer" onClick={(event) => event.stopPropagation()}>《服务条款》</a> 和 <a href={LEGAL_POLICY.routes.guidelines} target="_blank" rel="noopener noreferrer" onClick={(event) => event.stopPropagation()}>《社区准则》</a>，且已阅读并知悉 <a href={LEGAL_POLICY.routes.privacy} target="_blank" rel="noopener noreferrer" onClick={(event) => event.stopPropagation()}>《隐私政策》</a>。</label></div>{error ? <div id="legal-consent-error" className="auth-alert auth-alert--error" role="alert">{error}</div> : null}<div className="community-cta-row"><button className="community-button auth-button" type="submit" disabled={busy}>{busy ? "正在记录确认..." : "确认并继续"}</button><button className="community-button--secondary auth-button" type="button" onClick={() => void signOut()} disabled={busy}>退出登录</button></div></form></section>;
}
