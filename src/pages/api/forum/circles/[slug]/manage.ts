...
import { requireManagedCircleBySlug, jsonResponse, isCircleManager } from "../../../../../lib/server/circle-management";
...
export const PATCH: APIRoute = async ({ request, params, locals }) => {
  try {
    ...
    if (!isCircleManager(auth.circle.owner_id, auth.user.id, auth.profile.role)) {
      return jsonResponse({ error: "你没有权限修改这个圈子。" }, 403);
    }
    ...
    if ("name" in payload) {
      ...
      const { data: duplicateName, error: duplicateError } = await auth.client
        ...
        .limit(8);
      if (duplicateError) return jsonResponse({ error: duplicateError.message }, 500);
